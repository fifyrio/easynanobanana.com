...
export interface SymmetryAnalysisPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiFaceSymmetryExperienceProps {
  symmetryAnalysisPresets: SymmetryAnalysisPresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-face-symmetry/feature/before.png`;
const afterImage = `${R2_BASE}/ai-face-symmetry/feature/after.png`;

export default function AiFaceSymmetryExperience({ symmetryAnalysisPresets }: AiFaceSymmetryExperienceProps) {
  const t = useTranslations('aiFaceSymmetry');
  ...
  const [selectedAnalysis, setSelectedAnalysis] = useState<SymmetryAnalysisPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedAnalysis) {
      return 'Analyze facial symmetry by comparing the left and right halves of the face. Generate a professional symmetry report overlay on the photo showing: a vertical center line, symmetry percentage score (0-100%), left-right comparison grid lines on key facial landmarks (eyes, eyebrows, nose, lips, cheekbones, jawline), color-coded symmetry indicators (green for balanced, yellow for slight asymmetry, red for notable asymmetry), and an overall symmetry grade.';
    }

    const prompts: Record<string, string> = {
      'Overall Symmetry': 'Analyze the overall facial symmetry by comparing all left-right feature pairs. Generate a professional symmetry report overlay showing: a vertical center line, overall symmetry score (0-100%), left-right comparison lines on all facial landmarks, color-coded indicators (green/yellow/red), feature-by-feature breakdown bars for eyes, brows, nose, lips, cheeks, and jawline, plus an overall symmetry grade (A+ to F).',
      'Eye Symmetry': 'Analyze eye symmetry specifically. Generate a detailed eye symmetry report overlay showing: horizontal alignment lines between both eyes, measurements of eye width, eye opening height, inner/outer corner positions, eye tilt angle comparison, pupil distance from center, and an eye symmetry score (0-100%). Use color-coded markers and a focused close-up analysis panel.',
      'Eyebrow Symmetry': 'Analyze eyebrow symmetry specifically. Generate a detailed eyebrow symmetry report overlay showing: brow arch height comparison, brow thickness measurement, brow start/peak/end position alignment, brow angle comparison, and an eyebrow symmetry score (0-100%). Use color-coded markers highlighting areas of match and mismatch.',
      'Nose Symmetry': 'Analyze nose symmetry specifically. Generate a detailed nose symmetry report overlay showing: bridge alignment from center line, nostril width comparison, nostril shape comparison, nasal tip position, columella angle, and a nose symmetry score (0-100%). Use color-coded markers and measurement lines.',
      'Lip Symmetry': 'Analyze lip symmetry specifically. Generate a detailed lip symmetry report overlay showing: cupid\'s bow alignment, upper lip height comparison, lower lip volume comparison, lip corner height alignment, smile line analysis, and a lip symmetry score (0-100%). Use color-coded markers and measurement annotations.',
      'Cheekbone Symmetry': 'Analyze cheekbone symmetry specifically. Generate a detailed cheekbone symmetry report overlay showing: cheekbone prominence comparison, malar width measurement, cheek contour line comparison, mid-face volume balance, and a cheekbone symmetry score (0-100%). Use color-coded contour mapping and measurement lines.',
      'Jawline Symmetry': 'Analyze jawline symmetry specifically. Generate a detailed jawline symmetry report overlay showing: jaw angle comparison, mandibular contour lines, chin center alignment, gonial angle measurement, jaw width comparison, and a jawline symmetry score (0-100%). Use color-coded contour mapping and angular measurements.',
      'Golden Ratio': 'Analyze facial proportions against the golden ratio (Phi = 1.618). Generate a detailed golden ratio analysis overlay showing: phi mask overlay, key ratio measurements (face length:width, nose length:face length, eye spacing:nose width, lip width:nose width), percentage match to ideal phi proportions, golden spiral overlay on facial features, and an overall golden ratio score (0-100%). Use elegant gold-colored measurement lines and annotations.',
    };

    return prompts[selectedAnalysis.name] || prompts['Overall Symmetry'];
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedAnalysis ? { style: selectedAnalysis.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate face symmetry analysis.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Face symmetry analysis completed successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate face symmetry analysis. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Symmetry analysis preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {symmetryAnalysisPresets.map((preset) => {
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI face symmetry analysis result"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '⚖️', before: `${R2_BASE}/ai-face-symmetry/cases/case-1-before.png`, after: `${R2_BASE}/ai-face-symmetry/cases/case-1-after.png` },
              { key: 2, icon: '📐', before: `${R2_BASE}/ai-face-symmetry/cases/case-2-before.png`, after: `${R2_BASE}/ai-face-symmetry/cases/case-2-after.png` },
              { key: 3, icon: '✨', before: `${R2_BASE}/ai-face-symmetry/cases/case-3-before.png`, after: `${R2_BASE}/ai-face-symmetry/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Face Symmetry Preview"
          />
        </>
      )}
      <LoginModal
