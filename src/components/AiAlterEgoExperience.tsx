...
export interface AlterEgoStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiAlterEgoExperienceProps {
  alterEgoStylePresets: AlterEgoStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-alter-ego/feature/before.png`;
const afterImage = `${R2_BASE}/ai-alter-ego/feature/after.png`;

export default function AiAlterEgoExperience({ alterEgoStylePresets }: AiAlterEgoExperienceProps) {
  const t = useTranslations('aiAlterEgo');
  ...
  const [selectedStyle, setSelectedStyle] = useState<AlterEgoStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Keep the subject exactly as they are. No alter ego transformation.';
    }
    return `Transform the subject into a ${selectedStyle.name} alter ego. Apply the full visual aesthetic, styling, costume, and atmosphere associated with the ${selectedStyle.name} archetype. Keep their facial identity recognizable while dramatically reimagining their look and persona.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStyle ? { alterEgoStyle: selectedStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate alter ego preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Alter ego transformation applied successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate alter ego preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Alter ego style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {alterEgoStylePresets.map((preset) => {
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated alter ego preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎭', before: `${R2_BASE}/ai-alter-ego/cases/case-1-before.png`, after: `${R2_BASE}/ai-alter-ego/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-alter-ego/cases/case-2-before.png`, after: `${R2_BASE}/ai-alter-ego/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-alter-ego/cases/case-3-before.png`, after: `${R2_BASE}/ai-alter-ego/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
