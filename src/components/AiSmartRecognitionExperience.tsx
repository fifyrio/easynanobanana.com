...
export interface RecognitionStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiSmartRecognitionExperienceProps {
  recognitionStylePresets: RecognitionStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-smart-recognition/feature/before.png`;
const afterImage = `${R2_BASE}/ai-smart-recognition/feature/after.png`;

export default function AiSmartRecognitionExperience({ recognitionStylePresets }: AiSmartRecognitionExperienceProps) {
  const t = useTranslations('aiSmartRecognition');
  ...
  const [selectedStyle, setSelectedStyle] = useState<RecognitionStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Add a futuristic smart recognition HUD overlay to this image. Include dashed bounding boxes around key subjects, zoomed-in detail panels showing close-up areas, technical labels and data readouts, hex color codes, a timestamp, and an "ANALYSIS MODE" header. The overlay should look like a high-tech surveillance or AI analysis interface while keeping the original photo visible underneath.';
    }

    const prompts: Record<string, string> = {
      'Fashion Analysis': 'Add a tech HUD analysis overlay to this image focused on fashion and clothing recognition. Include dashed bounding boxes around clothing items and accessories, zoomed-in detail panels showing fabric textures, stitching, and materials. Add labels like "ANALYSIS MODE: CLOTHING & BIOMETRICS", a subject ID number, timestamp in the corner, and technical data readouts for each detected item. The overlay should look like an AI fashion scanner interface with neon-colored annotation lines.',
      'Face Recognition': 'Add a facial recognition analysis HUD overlay to this image. Place a recognition bounding box around the face with corner markers. Add circular zoomed-in detail panels for eyes, lips, hair, and skin texture with technical labels. Include facial landmark positioning data, biometric measurements, confidence scores, and a "FACIAL ANALYSIS" header. The overlay should look like a high-tech biometric scanner with glowing annotation lines and data readouts.',
      'Product Analysis': 'Add a product analysis HUD overlay to this image. Include dashed bounding boxes around the product, zoomed-in detail panels showing ingredients, textures, and surface details. Add labels like "PRODUCT ANALYSIS", a product ID code, hex color codes for dominant colors, composition data readouts, and a timestamp. The overlay should look like an AI product inspection scanner interface with clean tech annotations.',
      'Logo Detection': 'Add a brand and logo detection HUD overlay to this image. Include bounding boxes around any logos or brand elements, text recognition callouts, color analysis panels showing hex color codes and RGB values. Add labels like "LOGO ANALYSIS", brand identification data, font detection readouts, and design element breakdowns. The overlay should look like an AI brand recognition scanner with precise annotation lines.',
      'Full Body Scan': 'Add a full body biometric scan HUD overlay to this image. Include a full-body outline with measurement annotations, multiple bounding boxes for different body regions, zoomed-in detail panels for key areas. Add labels like "BIOMETRIC SCAN", body proportion data, posture analysis indicators, height and measurement estimates, and a grid overlay. The overlay should look like a futuristic body scanner interface.',
      'Surveillance Feed': 'Add a surveillance camera feed HUD overlay to this image. Include a "REC" indicator with blinking dot, timestamp in the corner, camera angle and focal length data, grid overlay lines, motion detection bounding boxes, night vision style green tint on edges. Add labels like "CAM-01", frame counter, and security classification markers. The overlay should look like authentic CCTV footage with all standard surveillance UI elements.',
      'Color Palette': 'Add a color extraction and analysis HUD overlay to this image. Include multiple color sample boxes with hex codes, RGB and HSL values for dominant colors, color proportion pie chart or bar indicators, gradient analysis lines. Add labels like "COLOR ANALYSIS", palette name suggestions, color harmony indicators, and contrast ratio data. The overlay should look like a professional color analysis tool interface.',
      'Technical Blueprint': 'Add a technical blueprint analysis HUD overlay to this image. Include engineering-style measurement lines with dimensions, cross-section indicators, material composition callouts, structural analysis annotations. Add labels like "TECHNICAL ANALYSIS", wireframe overlay elements, specification tables, and schematic-style dotted guidelines. The overlay should look like an engineering blueprint or CAD analysis interface with a blue-tinted technical aesthetic.',
    };

    return prompts[selectedStyle.name] || prompts['Fashion Analysis'];
  };
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate recognition overlay.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Smart recognition overlay completed successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate recognition overlay. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="smart-recognition-upload"
                ...
                {/* Recognition style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {recognitionStylePresets.map((preset) => {
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI smart recognition overlay result"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-smart-recognition.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🔍', before: `${R2_BASE}/ai-smart-recognition/cases/case-1-before.png`, after: `${R2_BASE}/ai-smart-recognition/cases/case-1-after.png` },
              { key: 2, icon: '📊', before: `${R2_BASE}/ai-smart-recognition/cases/case-2-before.png`, after: `${R2_BASE}/ai-smart-recognition/cases/case-2-after.png` },
              { key: 3, icon: '🎯', before: `${R2_BASE}/ai-smart-recognition/cases/case-3-before.png`, after: `${R2_BASE}/ai-smart-recognition/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Smart Recognition Preview"
          />
        </>
      )}
      <LoginModal
