...
export interface SkinColorPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiSkinColorExperienceProps {
  skinColorPresets: SkinColorPresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/feature/after.png';

export default function AiSkinColorExperience({ skinColorPresets }: AiSkinColorExperienceProps) {
  const t = useTranslations('aiSkinColor');
  ...
  const [selectedSkinColor, setSelectedSkinColor] = useState<SkinColorPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedSkinColor) {
      return 'Keep the subject\'s skin tone exactly as it is. No skin color modification.';
    }
    return `Transform the subject's skin tone to a realistic ${selectedSkinColor.name} complexion. The skin should look natural with proper undertones, not painted or artificial. Keep their identity, facial features, clothing, pose, and background identical.`;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const skinColorHint = selectedSkinColor
        ? `The requested skin color is "${selectedSkinColor.name}". Apply a natural, realistic skin tone transformation that looks genuine.`
        : '';
      const finalPrompt = `${promptText} ${skinColorHint} Deliver a photo-realistic skin color transformation powered by Easy Nano Banana. Preserve the subject's identity, clothing, and accessories. Avoid changing the background.`;
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedSkinColor ? { skinColor: selectedSkinColor.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate skin color preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Skin color transformation applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate skin color preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {skinColorPresets.map((preset) => {
                      const isSelected = selectedSkinColor?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedSkinColor(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedSkinColor
                        ? t('input.preset.summary.selected', { name: selectedSkinColor.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated skin color preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/cases/case-1-after.png' },
              { key: 2, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-color/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          <ShareModal
            ...
            imageUrl={generatedImage}
            description={description || 'AI Skin Color by Easy Nano Banana'}
          />
          <ImagePreviewModal
            isOpen={showPreviewModal}
            onClose={() => setShowPreviewModal(false)}
            imageUrl={generatedImage}
            title="AI Skin Color Preview"
          />
        </>
      )}
      <LoginModal
