...
export interface EyewearStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiEyewearTryonExperienceProps {
  eyewearStyles: EyewearStylePresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/feature/after.png';

export default function AiEyewearTryonExperience({ eyewearStyles }: AiEyewearTryonExperienceProps) {
  const t = useTranslations('aiEyewearTryon');
  ...
  const [selectedEyewear, setSelectedEyewear] = useState<EyewearStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedEyewear) {
      return 'Keep the subject exactly as they are. No eyewear transformation.';
    }
    return `Apply realistic ${selectedEyewear.name} eyewear to the subject. Place the glasses naturally on their face, fitting the frame style accurately to the ${selectedEyewear.name} design. Keep their identity, pose, clothing, and background identical.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedEyewear ? { eyewear: selectedEyewear.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate eyewear try-on preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Eyewear applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate eyewear try-on preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {eyewearStyles.map((preset) => {
                      const isSelected = selectedEyewear?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedEyewear(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedEyewear
                        ? t('input.preset.summary.selected', { name: selectedEyewear.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated eyewear try-on preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '👓', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/cases/case-1-after.png' },
              { key: 2, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-eyewear-tryon/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Eyewear Try-On Preview"
          />
        </>
      )}
      <LoginModal
