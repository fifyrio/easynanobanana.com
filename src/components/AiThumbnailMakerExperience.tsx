...
export interface ThumbnailMakerPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiThumbnailMakerExperienceProps {
  thumbnailPresets: ThumbnailMakerPresetAsset[];
}

const PROMPT_MAP: Record<string, string> = {
  'Clickbait': 'Transform this image into a viral clickbait YouTube thumbnail. Add bold, oversized text with bright yellow and red colors. Include dramatic arrows, circles highlighting key elements, and exaggerated expressions. Use high contrast, saturated colors, and a sense of urgency. Professional YouTube thumbnail layout optimized for maximum click-through rate.',
  'Minimalist': 'Transform this image into a clean minimalist YouTube thumbnail. Use a simple composition with lots of negative space, one or two bold sans-serif words, and a limited color palette. Focus on clarity and readability at small sizes. Modern, premium aesthetic with subtle shadows. Professional YouTube thumbnail layout.',
  'Cinematic': 'Transform this image into a cinematic movie-poster-style YouTube thumbnail. Apply dramatic lighting with lens flares, depth of field blur, and film color grading (teal and orange). Add bold cinematic title typography with metallic or embossed effects. Wide-format composition with epic atmosphere. Professional YouTube thumbnail layout.',
  'Gaming': 'Transform this image into an energetic gaming YouTube thumbnail. Add neon glow effects, glitch distortion, electric sparks, and vibrant RGB colors. Use bold angular typography with chrome or holographic effects. Include gaming-style HUD elements or controller icons. High-energy composition. Professional YouTube thumbnail layout.',
  'Tutorial': 'Transform this image into a clear tutorial/how-to YouTube thumbnail. Add step numbers, clean labeled annotations, and instructional graphics. Use a bright, trustworthy color palette with blue and white accents. Include "How To" or numbered step indicators. Clean sans-serif typography. Professional YouTube thumbnail layout.',
  'Vlog': 'Transform this image into a bright, lifestyle vlog YouTube thumbnail. Apply warm, vibrant color grading with golden sunlight effects. Add playful handwritten-style text with emojis or stickers. Create a friendly, inviting atmosphere with soft bokeh backgrounds. Travel or lifestyle aesthetic. Professional YouTube thumbnail layout.',
  'News/Reaction': 'Transform this image into a breaking news or reaction YouTube thumbnail. Add a news ticker or banner overlay, bold headline text in white on red or dark backgrounds. Include "BREAKING" or reaction emoji overlays. Split-screen composition with dramatic expressions. High contrast urgent design. Professional YouTube thumbnail layout.',
  'Retro Pop': 'Transform this image into a retro pop art YouTube thumbnail. Apply comic book halftone dots, bold black outlines, and vibrant pop art colors (bright pink, yellow, cyan). Add speech bubbles, starburst shapes, and vintage Ben-Day dot patterns. Roy Lichtenstein inspired aesthetic. Professional YouTube thumbnail layout.',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/feature/after.png';

export default function AiThumbnailMakerExperience({ thumbnailPresets }: AiThumbnailMakerExperienceProps) {
  const t = useTranslations('aiThumbnailMaker');
  ...
  const [selectedPreset, setSelectedPreset] = useState<ThumbnailMakerPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    const titleInstruction = thumbnailTitle.trim()
      ? ` The thumbnail title text is "${thumbnailTitle.trim()}". Display this text prominently on the thumbnail with bold, eye-catching typography that is readable at small sizes.`
      : '';
    if (!selectedPreset) {
      return `Transform this image into a professional YouTube thumbnail design. Create an eye-catching, click-worthy thumbnail with bold typography and compelling composition.${titleInstruction}`;
    }
    const base = PROMPT_MAP[selectedPreset.name] || `Transform this image into a ${selectedPreset.name} style YouTube thumbnail. Create a professional, click-worthy thumbnail with the ${selectedPreset.name} aesthetic.`;
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPreset ? { thumbnailStyle: selectedPreset.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Thumbnail generated successfully');
            await refreshProfile();
            return;
          } else {
            setError('Thumbnail generation completed but no result URL found.');
            return;
          }
        }

        if (statusData.status === 'failed') {
          setError(statusData.error || 'Thumbnail generation failed. Please try again.');
          return;
        }
      }

      setError('Thumbnail generation is taking longer than expected. Please check back later.');
    } catch (err: unknown) {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="thumbnail-maker-upload"
                ...
                <div>
                  <label htmlFor="thumbnail-title" className="block text-sm font-semibold text-slate-900 mb-2">
                    {t('input.thumbnailTitle.label')}
                  </label>
                  <input
                    type="text"
                    id="thumbnail-title"
                    ...
                    placeholder={t('input.thumbnailTitle.placeholder')}
                    maxLength={100}
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">{t('input.thumbnailTitle.hint')}</p>
                </div>
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎬', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/cases/case-1-after.png' },
              { key: 2, icon: '🎨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-thumbnail-maker/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
