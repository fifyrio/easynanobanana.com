...
export interface TeethWhiteningStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiTeethWhiteningExperienceProps {
  teethWhiteningStyles: TeethWhiteningStylePresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/feature/after.png';

export default function AiTeethWhiteningExperience({ teethWhiteningStyles }: AiTeethWhiteningExperienceProps) {
  const t = useTranslations('aiTeethWhitening');
  ...
  const [selectedTeethWhitening, setSelectedTeethWhitening] = useState<TeethWhiteningStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedTeethWhitening) {
      return 'Keep the subject exactly as they are. No teeth whitening transformation.';
    }
    return `Apply a realistic ${selectedTeethWhitening.name} teeth whitening effect to the subject. Whiten and brighten the teeth to match the ${selectedTeethWhitening.name} style while keeping the result natural and photo-realistic. Keep their identity, pose, clothing, and background identical.`;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const teethWhiteningHint = selectedTeethWhitening
        ? `The requested teeth whitening style is "${selectedTeethWhitening.name}". Apply professional, realistic whitening that enhances the subject's smile naturally.`
        : '';
      const finalPrompt = `${promptText} ${teethWhiteningHint} Deliver a photo-realistic teeth whitening transformation powered by Easy Nano Banana. Preserve the subject's identity, clothing, and accessories. Avoid changing the background.`;
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedTeethWhitening ? { teethWhitening: selectedTeethWhitening.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate teeth whitening preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Teeth whitening applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate teeth whitening preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="teeth-whitening-upload"
                ...
                {/* Teeth whitening style selector */}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {teethWhiteningStyles.map((preset) => {
                      const isSelected = selectedTeethWhitening?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedTeethWhitening(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedTeethWhitening
                        ? t('input.preset.summary.selected', { name: selectedTeethWhitening.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated teeth whitening preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-teeth-whitening.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🦷', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/cases/case-1-after.png' },
              { key: 2, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-teeth-whitening/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          <ShareModal
            ...
            imageUrl={generatedImage}
            description={description || 'AI Teeth Whitening by Easy Nano Banana'}
          />
          <ImagePreviewModal
            isOpen={showPreviewModal}
            onClose={() => setShowPreviewModal(false)}
            imageUrl={generatedImage}
            title="AI Teeth Whitening Preview"
          />
        </>
      )}
      <LoginModal
