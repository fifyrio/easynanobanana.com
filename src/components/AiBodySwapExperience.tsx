...
export interface BodySwapPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiBodySwapExperienceProps {
  swapPresets: BodySwapPresetAsset[];
}
...
const beforeImage = `${R2_BASE}/ai-body-swap/feature/before.png`;
const afterImage = `${R2_BASE}/ai-body-swap/feature/after.png`;

const PROMPT_MAP: Record<string, string> = {
  'Natural Blend': 'Place the person from the first image naturally onto the target body in the second image. Seamlessly blend face, skin tone, and lighting for a photorealistic result. Preserve the original face, facial expression, hairstyle, and recognizable identity. Match shadows, highlights, and ambient lighting between face and body.',
  'Perfect Skin Match': 'Place the person from the first image onto the target body in the second image with perfect skin tone matching. Carefully adjust the face skin color, undertone, and texture to match the body seamlessly. Ensure no visible boundary between face and body. Preserve facial expression and hairstyle.',
  'Athletic Fit': 'Place the person from the first image onto the athletic/fit body in the second image. Make the composite look natural with proper muscle tone and body proportions matching the face. Preserve facial identity, expression, and hairstyle. Ensure seamless skin tone blending.',
  'Fashion Editorial': 'Place the person from the first image onto the body in the second image with a high-fashion editorial quality. Professional fashion photography lighting, elegant pose integration, magazine-quality skin retouching. Preserve facial identity and expression perfectly.',
  'Professional Portrait': 'Place the person from the first image onto the body in the second image with a clean professional portrait style. Corporate/business quality, neutral lighting, polished appearance. Preserve facial identity, expression, and professionalism.',
  'Artistic Merge': 'Artistically merge the person from the first image onto the body in the second image. Create a slightly stylized, creative composite with enhanced colors and artistic lighting effects while maintaining facial identity and expression.',
  'Cinematic Look': 'Place the person from the first image onto the body in the second image with a cinematic film quality. Apply cinematic color grading, dramatic lighting, shallow depth of field effect. Preserve facial identity and expression while creating a movie-poster quality result.',
  'Cosplay Transform': 'Place the person from the first image onto the cosplay/costume body in the second image. Seamlessly integrate the face onto the costumed body, matching any makeup or themed elements. Preserve facial identity while enhancing the costume/character transformation.',
};

const ASPECT_RATIOS = ['1:1', '9:16', '16:9', '3:2', '2:3'] as const;

export default function AiBodySwapExperience({ swapPresets }: AiBodySwapExperienceProps) {
  const t = useTranslations('aiBodySwap');
  ...
  const [selectedPreset, setSelectedPreset] = useState<BodySwapPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    const baseClause = 'Using the 2 uploaded photos, swap the face from the first image onto the body in the second image.';
    const stylePrompt = selectedPreset ? PROMPT_MAP[selectedPreset.name] || '' : 'Place the person from the first image naturally onto the target body in the second image while preserving the original face, facial expression, hairstyle, and recognizable identity.';
    const instructionClause = swapInstruction.trim()
      ? ` Additional instructions: ${swapInstruction.trim()}`
      : '';
    return `${baseClause} ${stylePrompt}${instructionClause}`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          ...
          metadata: selectedPreset
            ? { swapStyle: selectedPreset.name, faceCount: imageUrls.length }
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate body swap.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('AI body swap generated successfully');
      ...
      if (err instanceof Error) {
        if (err.name === 'TypeError' && err.message.includes('fetch')) {
          ...
          setError(err.message || 'Failed to generate body swap.');
        }
      } else {
        setError('Failed to generate body swap.');
      }
    } finally {
  ...
  const slotLabelKey = (index: number): string => {
    if (index === 0) return 'input.upload.slots.face';
    return 'input.upload.slots.body';
  };

  const slotIsRequired = (_index: number) => true;
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-5 shadow-inner space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {slots.map((slot, index) => {
                      const inputId = `body-swap-upload-${index}`;
                ...
                {/* Swap instruction textarea */}
                <div className="rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-5 shadow-inner space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-slate-900">{t('input.swapInstruction.label')}</p>
                    <span className="text-[10px] text-slate-400">{t('input.swapInstruction.hint')}</span>
                  </div>
                  <textarea
                    value={swapInstruction}
                    onChange={(e) => setSwapInstruction(e.target.value.slice(0, 300))}
                    placeholder={t('input.swapInstruction.placeholder')}
                    rows={3}
                    maxLength={300}
                    className="w-full rounded-xl border border-[#FFE7A1] bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:border-[#F0A202] focus:outline-none focus:ring-1 focus:ring-[#F0A202] resize-none"
                  />
                </div>

                {/* Swap style preset selector */}
                <div>
                ...
                <div className="rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-5 shadow-inner space-y-3">
                  <p className="text-sm font-semibold text-slate-900">{t('input.aspectRatio.label')}</p>
                    ...
                    {ASPECT_RATIOS.map((ratio) => (
                      <button
                        ...
                        className={`rounded-xl px-4 py-2 text-sm font-semibold transition border-2 ${
                          selectedRatio === ratio
                            ? 'border-[#F0A202] bg-[#FFF4CC] text-slate-900 shadow-[0_6px_16px_rgba(240,162,2,0.25)]'
                            : 'border-transparent bg-white text-slate-500 hover:bg-[#FFF7DA]'
                        }`}
                      >
                        {ratio}
                      </button>
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated body swap preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🔄', before: `${R2_BASE}/ai-body-swap/cases/case-1-before.png`, after: `${R2_BASE}/ai-body-swap/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-body-swap/cases/case-2-before.png`, after: `${R2_BASE}/ai-body-swap/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-body-swap/cases/case-3-before.png`, after: `${R2_BASE}/ai-body-swap/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
