...
export interface ThreeDStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiImageTo3dExperienceProps {
  threeDStylePresets: ThreeDStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-image-to-3d/feature/before.png`;
const afterImage = `${R2_BASE}/ai-image-to-3d/feature/after.png`;

export default function AiImageTo3dExperience({ threeDStylePresets }: AiImageTo3dExperienceProps) {
  const t = useTranslations('aiImageTo3d');
  ...
  const [selectedStyle, setSelectedStyle] = useState<ThreeDStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Transform this image into a 3D rendered model. Convert the subject into a realistic 3D object with proper lighting, shadows, and depth. The result should look like a professional 3D render of the subject, placed on a clean studio background with soft ambient lighting.';
    }

    const prompts: Record<string, string> = {
      'Clay Render': 'Transform this image into a smooth matte clay sculpture 3D render. Convert the subject into a clay-like material with a soft, uniform matte finish in warm beige/gray tones. Show subtle form details through soft shadows and ambient occlusion. The result should look like an unfinished clay model in a 3D sculpting software, placed on a clean studio background with soft even lighting.',
      'Low Poly': 'Transform this image into a low-polygon 3D model render. Convert the subject into geometric faceted shapes with flat-shaded triangular faces, sharp angular edges, and simplified forms. Use a limited color palette derived from the original image. The result should look like a stylized low-poly 3D game asset with visible polygon edges, placed on a clean background with soft directional lighting.',
      'Isometric': 'Transform this image into an isometric 3D miniature scene. Convert the subject into a tiny 3D model viewed from a 30-degree isometric camera angle. Add a small platform or base underneath. The style should be cute, detailed, and miniature — like a tiny diorama or toy model. Clean white/light background, soft shadows, professional 3D render quality.',
      'Realistic 3D': 'Transform this image into a photorealistic 3D model render. Convert the subject into a highly detailed 3D object with accurate materials, textures, subsurface scattering, and physically-based rendering. Show realistic lighting with soft shadows, reflections, and ambient occlusion. The result should look indistinguishable from a real 3D product visualization, placed on a clean studio background.',
      'Cartoon 3D': 'Transform this image into a Pixar/Disney-style 3D cartoon character or object. Convert the subject with exaggerated proportions, smooth rounded surfaces, big expressive features, and vibrant saturated colors. Add stylized lighting with a warm rim light. The result should look like a character from a modern animated movie, rendered in high quality with a clean colorful background.',
      'Wireframe': 'Transform this image into a 3D wireframe mesh visualization. Convert the subject into a visible polygon mesh structure showing all edges and vertices as thin lines on a dark background. Use cyan/green/white wireframe lines against a dark navy or black background. Add subtle vertex points at intersections. The result should look like a 3D model being viewed in wireframe mode in professional 3D software.',
      'Voxel': 'Transform this image into a voxel-art 3D model in the style of Minecraft or CrossyRoad. Convert the subject into small cubic blocks/voxels, creating a blocky pixelated 3D representation. Use bright, flat colors for each voxel. The result should look like a charming voxel art model viewed at a slight angle, placed on a clean background with soft lighting and subtle shadows.',
      'Metallic': 'Transform this image into a shiny metallic 3D sculpture render. Convert the subject into a polished chrome or liquid metal material with mirror-like reflections, caustic light patterns, and dramatic specular highlights. The surface should look like molten silver or polished steel. Place on a reflective dark surface with dramatic studio lighting creating sharp reflections and contrast.',
    };

    return prompts[selectedStyle.name] || prompts['Clay Render'];
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStyle ? { style: selectedStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('3D model render completed successfully');
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedStyle
                        ? t('input.preset.summary.selected', { name: selectedStyle.name })
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🧊', before: `${R2_BASE}/ai-image-to-3d/cases/case-1-before.png`, after: `${R2_BASE}/ai-image-to-3d/cases/case-1-after.png` },
              { key: 2, icon: '🎮', before: `${R2_BASE}/ai-image-to-3d/cases/case-2-before.png`, after: `${R2_BASE}/ai-image-to-3d/cases/case-2-after.png` },
              { key: 3, icon: '✨', before: `${R2_BASE}/ai-image-to-3d/cases/case-3-before.png`, after: `${R2_BASE}/ai-image-to-3d/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
