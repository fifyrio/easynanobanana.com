...
export interface FatFilterPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiFatFilterExperienceProps {
  bodyTypePresets: FatFilterPresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/feature/after.png';

export default function AiFatFilterExperience({ bodyTypePresets }: AiFatFilterExperienceProps) {
  const t = useTranslations('aiFatFilter');
  ...
  const [selectedBodyType, setSelectedBodyType] = useState<FatFilterPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedBodyType) {
      return 'Keep the subject exactly as they are. No body transformation.';
    }
    if (selectedBodyType.name === 'Average') {
      return 'Keep the subject at their current average body shape. No changes to body weight or proportions. Maintain their natural physique exactly as it is.';
    }
    return `Transform the subject to have a realistic ${selectedBodyType.name} body type. Adjust face fullness, body proportions, and overall physique to match a ${selectedBodyType.name} build naturally. Keep their identity, clothing style, pose, and background identical.`;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const bodyTypeHint = selectedBodyType
        ? `The requested body type is "${selectedBodyType.name}". Apply a realistic, natural-looking body transformation that adjusts face and body proportions naturally.`
        : '';
      const finalPrompt = `${promptText} ${bodyTypeHint} Deliver a photo-realistic body transformation powered by Easy Nano Banana. Preserve the subject's identity, clothing, and accessories. Avoid changing the background.`;
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedBodyType ? { bodyType: selectedBodyType.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate fat filter preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Body type transformation applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate fat filter preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {bodyTypePresets.map((preset) => {
                      const isSelected = selectedBodyType?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedBodyType(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedBodyType
                        ? t('input.preset.summary.selected', { name: selectedBodyType.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated fat filter preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '⚖️', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/cases/case-1-after.png' },
              { key: 2, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-fat-filter/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          <ShareModal
            ...
            imageUrl={generatedImage}
            description={description || 'AI Fat Filter by Easy Nano Banana'}
          />
          <ImagePreviewModal
            isOpen={showPreviewModal}
            onClose={() => setShowPreviewModal(false)}
            imageUrl={generatedImage}
            title="AI Fat Filter Preview"
          />
        </>
      )}
      <LoginModal
