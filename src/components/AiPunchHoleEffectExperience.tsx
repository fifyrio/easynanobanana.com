...
export interface PunchHoleStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiPunchHoleEffectExperienceProps {
  punchHoleStylePresets: PunchHoleStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-punch-hole-effect/feature/before.png`;
const afterImage = `${R2_BASE}/ai-punch-hole-effect/feature/after.png`;

export default function AiPunchHoleEffectExperience({ punchHoleStylePresets }: AiPunchHoleEffectExperienceProps) {
  const t = useTranslations('aiPunchHoleEffect');
  ...
  const [selectedPunchHoleStyle, setSelectedPunchHoleStyle] = useState<PunchHoleStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedPunchHoleStyle) {
      return 'Apply a creative punch hole cutout effect to this photo, creating artistic shaped holes or overlays.';
    }
    const stylePrompts: Record<string, string> = {
      'Circle Cutout': 'Apply a circle punch hole cutout effect to this photo. Create clean, perfectly round holes punched through the image, revealing a contrasting background layer beneath. The circular cutouts should be strategically placed to create an artistic composition while maintaining the subject visibility. Sharp edges on the holes with subtle shadow depth.',
      'Heart Cutout': 'Apply a heart-shaped punch hole cutout effect to this photo. Create romantic heart-shaped holes punched through the image, revealing a contrasting or colorful background beneath. The heart cutouts vary in size and are artistically arranged. Clean edges with a slight 3D paper-punch feel.',
      'Star Cutout': 'Apply a star-shaped punch hole cutout effect to this photo. Create sharp, geometric star-shaped holes punched through the image, revealing a vibrant background layer. Stars vary in size from small to large, scattered across the composition. Crisp pointed edges with realistic paper-punch depth.',
      'Diamond Cutout': 'Apply a diamond-shaped punch hole cutout effect to this photo. Create elegant diamond/rhombus-shaped holes punched through the image in a geometric pattern. The diamond cutouts create a sophisticated lattice or scattered arrangement, revealing a contrasting background. Clean angular edges with subtle shadow.',
      'Multi Holes': 'Apply a multi-hole punch effect to this photo. Create numerous small circular holes punched in a dense, organized grid or pattern across the image, similar to a perforated sheet. The holes create a halftone-like visual effect where the image is visible through the pattern of small openings. Uniform hole size with consistent spacing.',
      'Stripe Overlay': 'Apply a stripe overlay punch effect to this photo. Create horizontal or vertical stripe cutouts through the image, alternating between visible strips of the photo and empty gaps. The stripes create a venetian blind or sliced effect, partially revealing and concealing the image. Clean parallel edges with even spacing.',
      'Torn Paper': 'Apply a torn paper punch hole effect to this photo. Create irregular, organic-shaped holes that look like paper has been torn or ripped away from the image. The edges are rough, fibrous, and natural-looking rather than clean-cut. Multiple torn openings of varying sizes reveal a textured background beneath.',
      'Polaroid Frame': 'Apply a Polaroid frame punch effect to this photo. Create a classic Polaroid instant photo frame overlay with the characteristic white border, wider at the bottom. The image appears as if viewed through a punched-out Polaroid shape, with the iconic instant camera aesthetic. Clean white frame edges with slight shadow.',
    };
    return stylePrompts[selectedPunchHoleStyle.name] || `Apply a ${selectedPunchHoleStyle.name} punch hole effect to this photo.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPunchHoleStyle ? { punchHoleStyle: selectedPunchHoleStyle.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate punch hole effect. Please try again.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Punch hole effect generated successfully');
        ...
        if (statusData.status === 'failed') {
          setError(statusData.error || 'Failed to generate punch hole effect. Please try again.');
          return;
        }
      }
      ...
      if (error.name === 'TypeError' && error.message?.includes('fetch')) {
        ...
        setError('Failed to generate punch hole effect. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Punch hole style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {punchHoleStylePresets.map((preset) => {
                      const isSelected = selectedPunchHoleStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedPunchHoleStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedPunchHoleStyle
                        ? t('input.preset.summary.selected', { name: selectedPunchHoleStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated punch hole effect"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-punch-hole-effect.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🕳️', before: `${R2_BASE}/ai-punch-hole-effect/cases/case-1-before.png`, after: `${R2_BASE}/ai-punch-hole-effect/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-punch-hole-effect/cases/case-2-before.png`, after: `${R2_BASE}/ai-punch-hole-effect/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-punch-hole-effect/cases/case-3-before.png`, after: `${R2_BASE}/ai-punch-hole-effect/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Punch Hole Effect Preview"
          />
        </>
      )}
      <LoginModal
