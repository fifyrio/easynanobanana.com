...
export interface GlowUpStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiGlowUpTestExperienceProps {
  glowUpStylePresets: GlowUpStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-glow-up-test/feature/before.png`;
const afterImage = `${R2_BASE}/ai-glow-up-test/feature/after.png`;

export default function AiGlowUpTestExperience({ glowUpStylePresets }: AiGlowUpTestExperienceProps) {
  const t = useTranslations('aiGlowUpTest');
  ...
  const [selectedStyle, setSelectedStyle] = useState<GlowUpStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Analyze this person\'s overall appearance and provide a comprehensive glow up assessment.';
    }
    return `Apply a ${selectedStyle.name} glow up transformation to this person. Enhance their natural features to achieve the ${selectedStyle.name} look while preserving their identity. Keep their pose, clothing, and background identical.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStyle ? { glowUpStyle: selectedStyle.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate glow up preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Glow up transformation applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate glow up preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated glow up preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '✨', before: `${R2_BASE}/ai-glow-up-test/cases/case-1-before.png`, after: `${R2_BASE}/ai-glow-up-test/cases/case-1-after.png` },
              { key: 2, icon: '💫', before: `${R2_BASE}/ai-glow-up-test/cases/case-2-before.png`, after: `${R2_BASE}/ai-glow-up-test/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-glow-up-test/cases/case-3-before.png`, after: `${R2_BASE}/ai-glow-up-test/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
