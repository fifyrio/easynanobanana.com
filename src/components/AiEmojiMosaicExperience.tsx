...
export interface EmojiMosaicPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiEmojiMosaicExperienceProps {
  mosaicPresets: EmojiMosaicPresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/feature/after.png';

export default function AiEmojiMosaicExperience({ mosaicPresets }: AiEmojiMosaicExperienceProps) {
  const t = useTranslations('aiEmojiMosaic');
  ...
  const [selectedMosaic, setSelectedMosaic] = useState<EmojiMosaicPresetAsset | null>(null);
  ...
  const PROMPT_MAP: Record<string, string> = {
    'Classic Grid': 'Transform the image into a classic emoji mosaic art piece where the entire photo is reconstructed using a uniform grid of emojis. Each emoji is carefully color-matched to represent the corresponding area of the original image. Clean grid layout with equal spacing.',
    'Dense Mosaic': 'Convert the image into a densely packed emoji mosaic with very small, tightly arranged emojis that preserve fine details and subtle color gradients. The emojis are packed so closely that the overall image appears almost photorealistic from a distance.',
    'Pixel Art': 'Transform the image into a retro pixel-art style emoji mosaic with large, blocky emoji tiles creating a deliberately pixelated look. The result should feel like a retro 8-bit video game version of the original photo made entirely from emojis.',
    'Color Pop': 'Create a vibrant, saturated emoji mosaic where the emojis are selected to maximize color intensity and contrast. The resulting mosaic should be eye-catchingly colorful with bold, vivid emoji choices that amplify the original colors.',
    'Monochrome': 'Transform the image into a monochrome emoji mosaic using only emojis in shades of a single color tone. The result is an elegant, tonal artwork that captures the subject through varying intensities of one color family.',
    'Neon Glow': 'Convert the image into a neon-glowing emoji mosaic on a dark background. The emojis should appear to emit bright neon light in electric blues, pinks, greens, and purples. The overall effect should look like a glowing sign made of emojis.',
    'Scattered': 'Transform the image into an artistic scattered emoji composition where emojis are randomly placed and overlapping at various sizes and angles. Rather than a strict grid, emojis are organically distributed to create an abstract, artistic interpretation.',
    'Vintage': 'Create a vintage-style emoji mosaic with muted, warm-toned colors reminiscent of old photographs. The emojis should be in sepia, amber, and faded tones. The overall result should feel nostalgic and retro, like an old postcard reimagined with emojis.',
  };

  const buildPrompt = () => {
    if (!selectedMosaic) {
      return 'Transform this image into an emoji mosaic art piece. Replace pixels with matching emojis arranged in a grid pattern.';
    }
    return PROMPT_MAP[selectedMosaic.name] || `Transform this image into a ${selectedMosaic.name} style emoji mosaic artwork.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedMosaic ? { mosaic: selectedMosaic.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate emoji mosaic.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Emoji mosaic created successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate emoji mosaic. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {mosaicPresets.map((preset) => {
                      const isSelected = selectedMosaic?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedMosaic(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedMosaic
                        ? t('input.preset.summary.selected', { name: selectedMosaic.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated emoji mosaic preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '😀', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/cases/case-1-after.png' },
              { key: 2, icon: '🎨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-emoji-mosaic/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
