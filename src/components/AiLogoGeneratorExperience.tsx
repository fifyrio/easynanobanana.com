...
export interface LogoStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiLogoGeneratorExperienceProps {
  logoStylePresets: LogoStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-logo-generator/feature/before.png`;
const afterImage = `${R2_BASE}/ai-logo-generator/feature/after.png`;

export default function AiLogoGeneratorExperience({ logoStylePresets }: AiLogoGeneratorExperienceProps) {
  const t = useTranslations('aiLogoGenerator');
  ...
  const [selectedLogoStyle, setSelectedLogoStyle] = useState<LogoStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedLogoStyle) {
      return 'Generate a creative logo design from this photo, applying a professional logo style to the subject.';
    }
    const stylePrompts: Record<string, string> = {
      'Minimalist': 'Generate a minimalist logo design from this photo. Create a clean, simple logo with minimal elements, generous whitespace, thin lines, and a refined aesthetic. The design should feature elegant simplicity, subtle geometry, and a modern, understated look with maximum impact from minimal visual elements.',
      'Vintage Badge': 'Generate a vintage badge logo design from this photo. Create a classic, retro-styled badge or emblem logo with ornate borders, vintage typography, weathered textures, and a timeless aesthetic. The design should feature banner ribbons, decorative frames, and an authentic old-world craftsmanship feel.',
      'Gradient Modern': 'Generate a gradient modern logo design from this photo. Create a contemporary logo with smooth color gradients, fluid shapes, and a vibrant, forward-looking aesthetic. The design should feature bold gradient transitions, sleek curves, and a dynamic, tech-forward appearance.',
      'Monogram': 'Generate a monogram logo design from this photo. Create an elegant letter-based logo with interlocking or stylized initials, refined typography, and a sophisticated aesthetic. The design should feature creative letterforms, balanced composition, and a luxury, high-end feel.',
      'Mascot': 'Generate a mascot logo design from this photo. Create a character-based logo with a distinctive, memorable mascot featuring bold colors, expressive personality, and a strong brand identity. The design should feature a stylized character with clear features and an energetic, approachable presence.',
      'Geometric': 'Generate a geometric logo design from this photo. Create a structured logo using precise geometric shapes, clean angles, mathematical proportions, and a balanced, orderly aesthetic. The design should feature polygons, circles, or abstract geometric patterns with sharp, defined edges.',
      'Hand Drawn': 'Generate a hand-drawn logo design from this photo. Create an artisanal, hand-crafted logo with organic lines, sketch-like quality, and a warm, authentic aesthetic. The design should feature imperfect line work, natural textures, and a personal, bespoke feel that conveys craftsmanship.',
      'Neon': 'Generate a neon logo design from this photo. Create a glowing neon-style logo with vibrant luminous colors, light effects, and a bold nightlife aesthetic. The design should feature neon tube-like outlines, bright glows, reflections, and an electric, eye-catching appearance against a dark background.',
    };
    return stylePrompts[selectedLogoStyle.name] || `Generate a ${selectedLogoStyle.name} logo design from this photo.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedLogoStyle ? { logoStyle: selectedLogoStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate logo design. Please try again.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Logo design generated successfully');
        ...
        if (statusData.status === 'failed') {
          setError(statusData.error || 'Failed to generate logo design. Please try again.');
          return;
        }
      }
      ...
      if (error.name === 'TypeError' && error.message?.includes('fetch')) {
        ...
        setError('Failed to generate logo design. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {logoStylePresets.map((preset) => {
                      const isSelected = selectedLogoStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedLogoStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedLogoStyle
                        ? t('input.preset.summary.selected', { name: selectedLogoStyle.name })
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-logo-generator.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎨', before: `${R2_BASE}/ai-logo-generator/cases/case-1-before.png`, after: `${R2_BASE}/ai-logo-generator/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-logo-generator/cases/case-2-before.png`, after: `${R2_BASE}/ai-logo-generator/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-logo-generator/cases/case-3-before.png`, after: `${R2_BASE}/ai-logo-generator/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Logo Generator Preview"
          />
        </>
      )}
      <LoginModal
