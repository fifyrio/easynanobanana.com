...
export interface TshirtDesignerPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiTshirtDesignerExperienceProps {
  tshirtPresets: TshirtDesignerPresetAsset[];
}

const PROMPT_MAP: Record<string, string> = {
  'Minimalist': 'Transform this image into a clean minimalist t-shirt design. Create a simple, elegant graphic with clean lines, minimal colors (black, white, or one accent color), plenty of negative space, and modern aesthetic. The design should be print-ready on a plain white t-shirt mockup.',
  'Vintage Retro': 'Transform this image into a vintage retro t-shirt design. Apply a distressed, worn-out texture with faded colors, retro typography, old-school screen print aesthetic from the 70s-80s. Add subtle grain, cracked ink effect, and vintage color palette (muted oranges, browns, yellows). Show on a vintage-wash t-shirt mockup.',
  'Pop Art': 'Transform this image into a bold Andy Warhol-style pop art t-shirt design. Use vibrant contrasting colors, halftone dot patterns, thick black outlines, and dramatic color blocks. Create a striking 4-panel or single bold graphic pop art composition. Show on a bright colored t-shirt mockup.',
  'Typography': 'Transform this image into an artistic typography-based t-shirt design. Incorporate the subject into creative text art, using letters, words, and font compositions to recreate the image. Mix serif and sans-serif fonts, varied sizes, and artistic text layout. Show on a clean t-shirt mockup.',
  'Watercolor': 'Transform this image into a beautiful watercolor art t-shirt design. Create soft, flowing watercolor splashes with blended colors, paint drips, wet-on-wet effects, and artistic brush strokes. Delicate and artistic with transparent color overlays. Show on a light-colored t-shirt mockup.',
  'Streetwear': 'Transform this image into an urban streetwear t-shirt design. Apply graffiti-style graphics, bold urban typography, spray paint textures, street art aesthetic with edgy composition. Use bold colors, drip effects, and underground culture vibes. Show on an oversized black t-shirt mockup.',
  'Abstract': 'Transform this image into an abstract geometric art t-shirt design. Break the subject into geometric shapes, triangles, polygons, and abstract forms with bold color fills. Modern abstract art composition with sharp edges and dynamic arrangement. Show on a clean t-shirt mockup.',
  'Neon Glow': 'Transform this image into a glowing neon light t-shirt design. Create the subject as neon tube outlines glowing against a dark background, with vivid cyan, magenta, and electric blue colors. Add realistic neon glow, light bloom, and reflection effects. Show on a black t-shirt mockup.',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/feature/after.png';

export default function AiTshirtDesignerExperience({ tshirtPresets }: AiTshirtDesignerExperienceProps) {
  const t = useTranslations('aiTshirtDesigner');
  ...
  const [selectedPreset, setSelectedPreset] = useState<TshirtDesignerPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedPreset) {
      return 'Transform this image into a creative t-shirt design. Create a print-ready graphic suitable for t-shirt printing with clean composition and appealing colors.';
    }
    return PROMPT_MAP[selectedPreset.name] || `Transform this image into a ${selectedPreset.name} style t-shirt design. Create a print-ready graphic with the ${selectedPreset.name} aesthetic.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPreset ? { tshirtStyle: selectedPreset.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate t-shirt design.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('T-shirt design generated successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate t-shirt design. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="tshirt-designer-upload"
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated t-shirt design preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '👕', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/cases/case-1-after.png' },
              { key: 2, icon: '🎨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-tshirt-designer/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI T-Shirt Design Preview"
          />
        </>
      )}
      <LoginModal
