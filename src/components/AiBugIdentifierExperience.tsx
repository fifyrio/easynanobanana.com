...
export interface BugAnalysisStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiBugIdentifierExperienceProps {
  bugAnalysisPresets: BugAnalysisStylePresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/feature/after.png';

export default function AiBugIdentifierExperience({ bugAnalysisPresets }: AiBugIdentifierExperienceProps) {
  const t = useTranslations('aiBugIdentifier');
  ...
  const [selectedAnalysis, setSelectedAnalysis] = useState<BugAnalysisStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedAnalysis) {
      return 'Analyze the uploaded bug/insect photo. Provide a general identification overlay with the insect name and basic information labels.';
    }

    const promptMap: Record<string, string> = {
      'Species ID': 'Analyze the uploaded bug/insect photo and generate an annotated version with a species identification overlay. Include scientific name, common name, family classification labels, and a confidence score bar.',
      'Danger Assessment': 'Analyze the uploaded bug/insect photo and generate an annotated version with a color-coded danger level overlay (green=harmless, yellow=nuisance, orange=stinger, red=dangerous). Include a safety rating bar and warning icons.',
      'Spider Analysis': 'Analyze the uploaded bug/insect photo and generate an annotated version with a spider-specific analysis overlay. Include venom level indicator, web type classification, and identification labels.',
      'Bite Analysis': 'Analyze the uploaded bug/insect photo and generate an annotated version with a bite identification overlay. Include suspected insect, symptom annotations, severity scale, and treatment recommendation labels.',
      'Pest Control': 'Analyze the uploaded bug/insect photo and generate an annotated version with a pest classification overlay. Include threat level, recommended control methods, and infestation risk indicators.',
      'Garden Friend': 'Analyze the uploaded bug/insect photo and generate an annotated version with a garden impact overlay. Include beneficial/harmful classification, pollinator status badge, and ecosystem role labels.',
      'Life Cycle': 'Analyze the uploaded bug/insect photo and generate an annotated version with a developmental stage overlay. Include stage identification (egg/larva/pupa/adult), approximate age, and lifecycle diagram annotations.',
      'Habitat Info': 'Analyze the uploaded bug/insect photo and generate an annotated version with a habitat information overlay. Include geographic range map, preferred environment labels, seasonal activity chart, and climate zone indicators.',
    };

    return promptMap[selectedAnalysis.name] || `Analyze the uploaded bug/insect photo and generate an annotated version with ${selectedAnalysis.name} analysis overlay.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedAnalysis ? { analysis: selectedAnalysis.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate bug analysis.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Bug analysis completed successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate bug analysis. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated bug analysis preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-bug-identifier.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '\uD83D\uDC1B', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/cases/case-1-after.png' },
              { key: 2, icon: '\u2728', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/cases/case-2-after.png' },
              { key: 3, icon: '\u26A1', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-bug-identifier/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Bug Identifier Preview"
          />
        </>
      )}
      <LoginModal
