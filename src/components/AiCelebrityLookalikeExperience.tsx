...
export interface CelebrityPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiCelebrityLookalikeExperienceProps {
  celebrityPresets: CelebrityPresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-celebrity-lookalike/feature/before.png`;
const afterImage = `${R2_BASE}/ai-celebrity-lookalike/feature/after.png`;

const PROMPT_MAP: Record<string, string> = {
  'A-List Hollywood': 'Transform the subject to look like a top-tier A-list Hollywood movie star: glamorous lighting, polished red-carpet hair and makeup, confident magazine-cover expression, sharp jawline, glowing skin. Subtle facial refinement toward classic Hollywood beauty ideals while keeping the subject recognizable. Preserve clothing and background.',
  'K-Pop Idol': 'Transform the subject to look like a K-Pop idol: porcelain glass skin, defined V-shape jawline, large bright eyes, soft natural-pink lip, perfectly styled trendy K-pop hairstyle, studio idol photoshoot lighting. Keep the subject\'s core identity recognizable. Preserve clothing and background.',
  'Bollywood Star': 'Transform the subject to look like a Bollywood film star: warm golden-hour lighting, rich dramatic makeup, defined kohl-rimmed eyes, glowing tan skin, glamorous hairstyle, cinematic romantic expression. Keep the subject\'s core identity recognizable. Preserve clothing and background.',
  'Hollywood Classic': 'Transform the subject to look like a 1950s Hollywood golden-age icon: vintage glamour lighting, classic red lip, elegant retro hairstyle, soft pearlescent skin, sophisticated old-Hollywood expression. Black-and-white feel but in color. Keep the subject recognizable. Preserve clothing and background.',
  'Movie Hero': 'Transform the subject to look like an action-movie leading hero: chiseled features, strong jawline, dramatic side lighting, intense determined expression, slightly weathered heroic look, cinematic color grade. Keep the subject\'s core identity recognizable. Preserve clothing and background.',
  'Fashion Icon': 'Transform the subject to look like a high-fashion runway icon: editorial Vogue-style lighting, sharp avant-garde makeup, sculpted cheekbones, intense fashion-photo gaze, polished editorial hairstyle. Keep the subject\'s core identity recognizable. Preserve clothing and background.',
  'Pop Star': 'Transform the subject to look like a chart-topping pop star: youthful glowing skin, vibrant trendy makeup, dynamic styled hair, energetic confident expression, music-video lighting. Keep the subject\'s core identity recognizable. Preserve clothing and background.',
  'Indie Charm': 'Transform the subject to look like an indie-film leading actor: natural soft daylight, minimal makeup, authentic candid expression, effortlessly tousled hair, warm filmic color grade. Keep the subject\'s core identity recognizable. Preserve clothing and background.',
};

export default function AiCelebrityLookalikeExperience({ celebrityPresets }: AiCelebrityLookalikeExperienceProps) {
  const t = useTranslations('aiCelebrityLookalike');
  ...
  const [selectedCelebrity, setSelectedCelebrity] = useState<CelebrityPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedCelebrity) {
      return 'Subtly enhance the subject toward a generic celebrity-like polish: cleaner skin, brighter eyes, glamour studio lighting. Keep the subject\'s identity, clothing, and background unchanged.';
    }
    return PROMPT_MAP[selectedCelebrity.name] || `Transform the subject to look like a ${selectedCelebrity.name} celebrity. Keep their core identity recognizable. Preserve clothing and background.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedCelebrity ? { celebrity: selectedCelebrity.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate celebrity lookalike preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Celebrity lookalike generated successfully');
      ...
      if (err instanceof Error) {
        if (err.name === 'TypeError' && err.message.includes('fetch')) {
          ...
          setError(err.message || 'Failed to generate celebrity lookalike preview.');
        }
      } else {
        setError('Failed to generate celebrity lookalike preview.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {celebrityPresets.map((preset) => {
                      const isSelected = selectedCelebrity?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedCelebrity(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedCelebrity
                        ? t('input.preset.summary.selected', { name: selectedCelebrity.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated celebrity lookalike preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-celebrity-lookalike.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🌟', before: `${R2_BASE}/ai-celebrity-lookalike/cases/case-1-before.png`, after: `${R2_BASE}/ai-celebrity-lookalike/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-celebrity-lookalike/cases/case-2-before.png`, after: `${R2_BASE}/ai-celebrity-lookalike/cases/case-2-after.png` },
              { key: 3, icon: '🎬', before: `${R2_BASE}/ai-celebrity-lookalike/cases/case-3-before.png`, after: `${R2_BASE}/ai-celebrity-lookalike/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Celebrity Lookalike Preview"
          />
        </>
      )}
      <LoginModal
