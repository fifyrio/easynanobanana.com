...
export interface ComicFrameStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiComicFrameExperienceProps {
  comicFramePresets: ComicFrameStylePresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/feature/after.png';

export default function AiComicFrameExperience({ comicFramePresets }: AiComicFrameExperienceProps) {
  const t = useTranslations('aiComicFrame');
  ...
  const [selectedComicFrame, setSelectedComicFrame] = useState<ComicFrameStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedComicFrame) {
      return 'Transform the uploaded photo into a comic book illustration. Use bold ink outlines, cel-shading, and vibrant colors. Keep the subject\'s facial features recognizable.';
    }
    return `Transform the uploaded photo into a ${selectedComicFrame.name} comic book illustration. Use bold ink outlines, cel-shading, and a themed ${selectedComicFrame.name}-style background and costume. Add comic text elements like speech bubbles or sound effects that fit the ${selectedComicFrame.name} genre. Keep the subject's facial features recognizable while stylizing them in the comic art style.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedComicFrame ? { comicFrame: selectedComicFrame.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate comic frame.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Comic frame applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate comic frame. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {comicFramePresets.map((preset) => {
                      const isSelected = selectedComicFrame?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedComicFrame(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedComicFrame
                        ? t('input.preset.summary.selected', { name: selectedComicFrame.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated comic frame preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '\uD83D\uDDBC\uFE0F', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/cases/case-1-after.png' },
              { key: 2, icon: '\u2728', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/cases/case-2-after.png' },
              { key: 3, icon: '\u26A1', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-comic-frame/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
