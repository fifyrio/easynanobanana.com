...
interface AiKissExperienceProps {
...
const SAMPLE_BEFORE_IMAGE =
  'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/feature/sample-couple.jpg';
const SAMPLE_AFTER_VIDEO =
  'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/feature/sample-kiss.mp4';

const CREDITS_REQUIRED = 100;
const POLL_INTERVAL_MS = 5000;
const POLL_MAX_ATTEMPTS = 48; // 4 min ceiling — Seedance 4s 480p typ. completes < 2 min

export default function AiKissExperience({ presets }: AiKissExperienceProps) {
  const t = useTranslations('aiKiss');
  const { user, profile, refreshProfile } = useAuth();

  const [slotA, setSlotA] = useState<ImageSlot>(EMPTY_SLOT);
  const [slotB, setSlotB] = useState<ImageSlot>(EMPTY_SLOT);
  const [aspectRatio, setAspectRatio] = useState<AspectRatio>('9:16');
  const [selectedPreset, setSelectedPreset] = useState<AiKissPreset | null>(
    presets[0] ?? null
  );
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <a
                        ...
                        download={`ai-kiss-${selectedPreset?.id ?? 'video'}.mp4`}
                        target="_blank"
        ...
        {/* Benefits — before/after case cards */}
            ...
            {[
              {
                key: 1,
                icon: '💞',
                before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/preset/soft-kiss.jpg',
                after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/preset/passionate-kiss.jpg',
              },
              {
                key: 2,
                icon: '✨',
                before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/preset/forehead-kiss.jpg',
                after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/preset/cheek-kiss.jpg',
              },
              {
                key: 3,
                icon: '🎬',
                before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/preset/first-kiss.jpg',
                after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-kiss/preset/slow-motion-kiss.jpg',
              },
            ].map((card, index) => (
              <div
                key={card.key}
        ...
        <section className="bg-white text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {(['anniversary', 'valentine', 'surprise', 'tribute', 'tiktok', 'wedding', 'longDistance', 'fandom'] as const).map((key) => (
              <div
                key={key}
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] to-white text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 grid gap-6 md:grid-cols-3">
            {(['seedance', 'identity', 'tiktok', 'fast', 'commercial', 'privacy'] as const).map((key) => (
              <div
                key={key}
