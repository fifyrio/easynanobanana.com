...
export interface AgePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiAgeFilterExperienceProps {
...
export default function AiAgeFilterExperience({ agePresets }: AiAgeFilterExperienceProps) {
  ...
  const [selectedAge, setSelectedAge] = useState<AgePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedAge) {
      return 'Keep the subject exactly as they are. No age transformation.';
    }
    return `Transform the subject to look like they are approximately ${selectedAge.age} years old (${selectedAge.name}). Adjust facial features, skin texture, and hair naturally to match this age. Keep their identity, pose, clothing, and background identical.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedAge ? { age: selectedAge.name, ageValue: selectedAge.age } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate age preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Age filter applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate age preview. Please try again.');
      }
    } finally {
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {agePresets.map((preset) => {
                      const isSelected = selectedAge?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedAge(isSelected ? null : preset)}
                        ...
                        >
                          ...
                          <div className="text-[9px] text-[#C69312]">{preset.age}</div>
                        </button>
                      );
                    })}
                  </div>
                      ...
                      {agePresets.map((preset) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-[10px] text-[#C69312]">{preset.age}</div>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedAge
                        ? t('input.preset.summary.selected', { name: selectedAge.name, age: selectedAge.age })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated age filter preview"
                    fill
