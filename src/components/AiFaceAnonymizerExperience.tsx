...
export interface AnonymizerStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiFaceAnonymizerExperienceProps {
  anonymizerStylePresets: AnonymizerStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-face-anonymizer/feature/before.png`;
const afterImage = `${R2_BASE}/ai-face-anonymizer/feature/after.png`;

export default function AiFaceAnonymizerExperience({ anonymizerStylePresets }: AiFaceAnonymizerExperienceProps) {
  const t = useTranslations('aiFaceAnonymizer');
  ...
  const [selectedStyle, setSelectedStyle] = useState<AnonymizerStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Anonymize this person\'s face by generating a synthetic AI look-alike face that preserves the general appearance (age, gender, ethnicity, hair color) but is a completely different person. The result should look like a real photo of a different person, not the original individual.';
    }

    const prompts: Record<string, string> = {
      'Natural Look-alike': 'Replace this person\'s face with an AI-generated synthetic face that closely resembles the original in age, gender, ethnicity, hair style, and overall vibe — but is a completely different person. The result must look like a natural, real photograph of someone else. Preserve pose, clothing, and background exactly.',
      'Subtle Disguise': 'Apply subtle but effective anonymization to this person\'s face. Change key identifying features — slightly alter eye shape, nose bridge width, jawline contour, lip shape, and eyebrow arch — just enough to make the person unrecognizable while keeping the overall look natural and realistic. Preserve pose, clothing, and background.',
      'Full Replace': 'Completely replace this person\'s face with an entirely different AI-generated face. The new face should be a different person entirely — different bone structure, different features, different look — while matching the general age range and maintaining a natural photographic quality. Preserve pose, clothing, and background.',
      'Pixel Mosaic': 'Apply a strong pixelation/mosaic effect to this person\'s face area only, creating large blocky pixels that completely obscure all facial features and make identification impossible. The pixelation should cover the entire face from forehead to chin, ear to ear. Keep the rest of the image sharp and unchanged.',
      'Gaussian Blur': 'Apply a heavy Gaussian blur effect to this person\'s face area only, creating a smooth, soft blur that completely obscures all facial features and makes identification impossible. The blur should cover the entire face from forehead to chin, ear to ear. Keep the rest of the image sharp and unchanged.',
      'Silhouette': 'Replace this person\'s face area with a solid dark silhouette shape. The silhouette should be a smooth, solid black or very dark shape that follows the outline of the head and face, completely obscuring all features. Keep the rest of the image unchanged and sharp.',
      'Digital Mask': 'Overlay a futuristic digital mask effect on this person\'s face. Apply a glowing, neon-lined geometric mask pattern with holographic elements covering all facial features. The mask should look like a high-tech privacy filter with subtle light effects and digital grid lines. Keep the rest of the image unchanged.',
      'Oil Paint': 'Transform this person\'s face into an artistic oil painting style that obscures their true identity. Apply thick, visible brush strokes, impasto texture, and painterly color blending to all facial features. The face should look like a hand-painted portrait — recognizably a face but impossible to identify the actual person. Keep the rest of the image as a photograph.',
    };

    return prompts[selectedStyle.name] || prompts['Natural Look-alike'];
  };
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate anonymized result.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Face anonymization completed successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate anonymized result. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="face-anonymizer-upload"
                ...
                {/* Anonymizer style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {anonymizerStylePresets.map((preset) => {
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI face anonymization result"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-face-anonymizer.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎭', before: `${R2_BASE}/ai-face-anonymizer/cases/case-1-before.png`, after: `${R2_BASE}/ai-face-anonymizer/cases/case-1-after.png` },
              { key: 2, icon: '🔒', before: `${R2_BASE}/ai-face-anonymizer/cases/case-2-before.png`, after: `${R2_BASE}/ai-face-anonymizer/cases/case-2-after.png` },
              { key: 3, icon: '✨', before: `${R2_BASE}/ai-face-anonymizer/cases/case-3-before.png`, after: `${R2_BASE}/ai-face-anonymizer/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Face Anonymizer Preview"
          />
        </>
      )}
      <LoginModal
