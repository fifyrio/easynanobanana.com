...
export interface AttractivenessStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiAttractivenessTestExperienceProps {
  attractivenessStylePresets: AttractivenessStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-attractiveness-test/feature/before.png`;
const afterImage = `${R2_BASE}/ai-attractiveness-test/feature/after.png`;

export default function AiAttractivenessTestExperience({ attractivenessStylePresets }: AiAttractivenessTestExperienceProps) {
  const t = useTranslations('aiAttractivenessTest');
  ...
  const [selectedStyle, setSelectedStyle] = useState<AttractivenessStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Analyze the subject\'s facial features and provide an overall attractiveness assessment.';
    }
    return `Analyze the subject's facial features and provide a detailed ${selectedStyle.name} attractiveness assessment. Highlight specific features relevant to ${selectedStyle.name}. Keep the subject's identity, pose, clothing, and background identical.`;
  };
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate attractiveness test result.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Attractiveness test completed successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate attractiveness test result. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="attractiveness-test-upload"
                ...
                {/* Attractiveness style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {attractivenessStylePresets.map((preset) => {
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI attractiveness test result"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-attractiveness-test.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '✨', before: `${R2_BASE}/ai-attractiveness-test/cases/case-1-before.png`, after: `${R2_BASE}/ai-attractiveness-test/cases/case-1-after.png` },
              { key: 2, icon: '🌟', before: `${R2_BASE}/ai-attractiveness-test/cases/case-2-before.png`, after: `${R2_BASE}/ai-attractiveness-test/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-attractiveness-test/cases/case-3-before.png`, after: `${R2_BASE}/ai-attractiveness-test/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Attractiveness Test Preview"
          />
        </>
      )}
      <LoginModal
