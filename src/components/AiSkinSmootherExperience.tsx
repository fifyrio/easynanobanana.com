...
export interface SkinSmootherStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiSkinSmootherExperienceProps {
  skinSmootherPresets: SkinSmootherStylePresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/feature/after.png';

export default function AiSkinSmootherExperience({ skinSmootherPresets }: AiSkinSmootherExperienceProps) {
  const t = useTranslations('aiSkinSmoother');
  ...
  const [selectedPreset, setSelectedPreset] = useState<SkinSmootherStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedPreset) {
      return 'Smooth and enhance the skin texture of the subject. Remove minor blemishes and imperfections while keeping the overall appearance natural.';
    }
    return `Apply a ${selectedPreset.name} skin smoothing effect to the subject. Enhance skin texture with the ${selectedPreset.name} style while preserving facial features, identity, pose, clothing, and background.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPreset ? { skinSmoother: selectedPreset.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate skin smoothing preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Skin smoothing applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate skin smoothing preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {skinSmootherPresets.map((preset) => {
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🧴', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/cases/case-1-after.png' },
              { key: 2, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-smoother/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Skin Smoother Preview"
          />
        </>
      )}
      <LoginModal
