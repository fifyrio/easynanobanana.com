...
export interface SkinAnalysisStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiSkinAnalyzerExperienceProps {
  skinAnalysisPresets: SkinAnalysisStylePresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/feature/after.png';

export default function AiSkinAnalyzerExperience({ skinAnalysisPresets }: AiSkinAnalyzerExperienceProps) {
  const t = useTranslations('aiSkinAnalyzer');
  ...
  const [selectedAnalysis, setSelectedAnalysis] = useState<SkinAnalysisStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedAnalysis) {
      return 'Add a comprehensive skin health overlay with overall score out of 100, multi-concern breakdown bars (acne, wrinkles, pores, hydration, radiance, firmness), color-coded zone mapping, skin type badge, semi-transparent diagnostic panel on the uploaded face photo.';
    }

    const prompts: Record<string, string> = {
      'Full Skin Report': 'Add a comprehensive skin health diagnostic overlay on the uploaded face photo. Include an overall skin health score out of 100, multi-concern breakdown bars for acne, wrinkles, pores, hydration, radiance, and firmness. Add color-coded zone mapping highlighting different facial areas, a skin type badge (oily/dry/combination/normal/sensitive), and a semi-transparent diagnostic panel with all metrics clearly visible.',
      'Acne Analysis': 'Add an acne diagnostic overlay on the uploaded face photo. Include an acne severity score, acne type labels (comedonal, inflammatory, cystic) pinpointing affected areas, an affected zone heat map with red/orange/yellow intensity gradients, a breakout count indicator showing total detected blemishes, and a treatment urgency badge.',
      'Wrinkle Map': 'Add a wrinkle analysis overlay on the uploaded face photo. Include depth mapping lines tracing wrinkle paths across forehead, around eyes, and nasolabial folds. Add crow\'s feet, forehead lines, and nasolabial fold individual scoring gauges, an estimated skin age badge, a collagen health meter, and a before/prevention tips panel overlay.',
      'Pore Analysis': 'Add a pore diagnostic overlay on the uploaded face photo. Include a pore density heat map focused on the T-zone (forehead, nose, chin), a pore size distribution chart, an average pore size indicator in micrometers, clogged pore markers with small circles, and a skin texture score badge.',
      'Dark Circle Check': 'Add an under-eye analysis overlay on the uploaded face photo. Include a dark circle severity gauge, pigmentation type label (vascular, melanin, or structural), an eye bag assessment indicator, a hydration level indicator for the under-eye area, and a concealer recommendation badge.',
      'Moisture Level': 'Add a hydration analysis overlay on the uploaded face photo. Include a moisture percentage gauge showing overall facial hydration, dry zone mapping with red (very dry), yellow (slightly dry), and green (hydrated) indicators across facial regions, a skin barrier health score, dehydration warning badges for critical areas, and a recommended hydration level bar.',
      'Skin Tone & Radiance': 'Add a tone analysis overlay on the uploaded face photo. Include a skin tone evenness score, a radiance level meter, pigmentation spot markers with small circles highlighting uneven areas, UV damage indicators, a color uniformity heat map across the face, and a glow score badge.',
      'Skin Age': 'Add an age analysis overlay on the uploaded face photo. Include a large "Skin Age: 28" vs "Actual Age: 32" comparison badge (use realistic estimated values based on the face), an aging factors breakdown showing sun damage, hydration, elasticity, and lifestyle scores, an age meter gauge visualization, and a skin vitality score.',
    };

    return prompts[selectedAnalysis.name] || prompts['Full Skin Report'];
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedAnalysis ? { skinAnalysis: selectedAnalysis.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate skin analysis.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Skin analysis completed successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate skin analysis. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {skinAnalysisPresets.map((preset) => {
                      const isSelected = selectedAnalysis?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedAnalysis(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedAnalysis
                        ? t('input.preset.summary.selected', { name: selectedAnalysis.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated skin analysis preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🧴', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/cases/case-1-after.png' },
              { key: 2, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-skin-analyzer/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Skin Analyzer Preview"
          />
        </>
      )}
      <LoginModal
