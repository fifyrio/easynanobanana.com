...
export interface OfficeStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiOfficeDesignExperienceProps {
  stylePresets: OfficeStylePresetAsset[];
}

type OfficeType = 'Open Office' | 'Private Office' | 'Conference Room' | 'Reception' | 'Coworking Space' | 'Executive Suite';

const OFFICE_TYPES: OfficeType[] = ['Open Office', 'Private Office', 'Conference Room', 'Reception', 'Coworking Space', 'Executive Suite'];

const OFFICE_TYPE_I18N: Record<OfficeType, string> = {
...
const STYLE_PROMPT_MAP: Record<string, string> = {
  'Modern': 'a modern office design — clean lines, neutral palette with bold accent colors, ergonomic furniture, glass partitions, sleek workstations, statement pendant lighting',
  'Industrial': 'an industrial office design — exposed brick and ductwork, raw concrete floors, black metal framing, Edison-bulb lighting, reclaimed wood desks, open ceiling',
  'Tech-Integrated': 'a tech-integrated office design — smart screens and video walls, sit-stand desks with cable management, ambient LED accent lighting, acoustic panels, futuristic ergonomic chairs',
  'Contemporary': 'a contemporary office design — warm neutral tones, curved soft seating, layered textures, designer lighting, biophilic touches, polished collaborative zones',
  'Minimalist': 'a minimalist office design — monochrome white and gray palette, uncluttered desks, hidden storage, abundant negative space, a single sculptural focal piece, clean geometry',
  'Scandinavian': 'a Scandinavian office design — light wood floors, white walls, soft neutral textiles, functional minimal furniture, indoor plants, abundant natural light, cozy hygge feel',
  'Mid-Century': 'a mid-century modern office design — walnut wood furniture, tapered legs, warm retro palette, geometric patterns, brass accents, classic lounge chairs',
  'Transitional': 'a transitional office design — a balanced blend of traditional and modern, neutral sophisticated palette, mix of classic and contemporary furniture, refined symmetry, elegant lighting',
  'Biophilic': 'a biophilic office design — living green walls, abundant potted plants, natural wood and stone materials, large windows with daylight, organic shapes, calming earthy palette',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/feature/after.png';

export default function AiOfficeDesignExperience({ stylePresets }: AiOfficeDesignExperienceProps) {
  const t = useTranslations('aiOfficeDesign');
  ...
  const [selectedStyle, setSelectedStyle] = useState<OfficeStylePresetAsset | null>(stylePresets[0] ?? null);
  const [officeType, setOfficeType] = useState<OfficeType>('Open Office');
  ...
  const buildPrompt = () => {
    const style = selectedStyle ?? stylePresets[0];
    if (!style) {
      return `Redesign this ${officeType.toLowerCase()} with new furniture and decor while preserving the architecture.`;
    }
    const styleDetail = STYLE_PROMPT_MAP[style.name] || `a ${style.name.toLowerCase()} office design`;
    return `Restyle this ${officeType.toLowerCase()} into ${styleDetail}. Replace all furniture, workstations, decor, lighting, flooring, and surface materials to match the chosen style. CRITICAL constraints: preserve the exact same room architecture — walls, windows, doors, ceiling, floor layout, room dimensions, viewpoint and camera angle must stay perfectly identical. The result must look like the exact same physical office space photographed from the exact same angle, but completely redesigned in the chosen style.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStyle ? { style: selectedStyle.name, officeType } : { officeType },
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate redesigned office.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Office redesigned successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate redesigned office. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Office type selector (chips) */}
                <div>
                  <p className="text-sm font-semibold text-slate-900 mb-2">{t('input.officeType.label')}</p>
                  <div className="flex flex-wrap gap-2">
                    {OFFICE_TYPES.map((ot) => {
                      const isSelected = officeType === ot;
                      return (
                        <button
                          key={ot}
                          type="button"
                          onClick={() => setOfficeType(ot)}
                        ...
                        >
                          {t(`input.officeType.options.${OFFICE_TYPE_I18N[ot]}`)}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div>
                    ...
                    {stylePresets.map((preset) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => setSelectedStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedStyle
                        ? t('input.preset.summary.selected', {
                            style: selectedStyle.name,
                            officeType: t(`input.officeType.options.${OFFICE_TYPE_I18N[officeType]}`),
                          })
            ...
            <div className="relative">
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                <div
                  ref={comparisonRef}
                  className="relative w-full overflow-hidden rounded-[28px] bg-gray-200 select-none aspect-[4/3]"
                ...
                >
                  <Image
                    ...
                    alt="AI redesigned office preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🏢', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/cases/case-1-after.png' },
              { key: 2, icon: '💼', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-office-design/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
                  ...
                  <div className="relative rounded-[28px] overflow-hidden border border-[#FFE7A1] bg-[#FFF3B2]/40 aspect-[4/3] flex items-center justify-center">
                    <div className="grid grid-cols-2 gap-2 p-3 w-full h-full">
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.before} alt="Before redesign" className="w-full h-full object-cover" />
                        <span className="absolute bottom-1 left-1 bg-black/50 text-white text-xs px-1.5 py-0.5 rounded">Before</span>
                      </div>
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.after} alt="After redesign" className="w-full h-full object-cover" />
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Redesigned Office Preview"
          />
        </>
      )}
      <LoginModal
