...
export interface MemeStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiMemeGeneratorExperienceProps {
  memeStylePresets: MemeStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-meme-generator/feature/before.png`;
const afterImage = `${R2_BASE}/ai-meme-generator/feature/after.png`;

export default function AiMemeGeneratorExperience({ memeStylePresets }: AiMemeGeneratorExperienceProps) {
  const t = useTranslations('aiMemeGenerator');
  ...
  const [selectedMemeStyle, setSelectedMemeStyle] = useState<MemeStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedMemeStyle) {
      return 'Generate a creative meme from this photo, applying a popular meme style to the subject.';
    }
    const stylePrompts: Record<string, string> = {
      'Classic Meme': 'Generate a classic meme from this photo. Apply the iconic Impact font white text with black outline style, positioned at the top and bottom of the image. The design should feature bold, humorous text placement with the classic meme aesthetic that dominated early internet culture.',
      'Deep Fried': 'Generate a deep fried meme from this photo. Apply extreme JPEG compression artifacts, oversaturated colors, lens flare effects, and emoji overlays. The image should look like it has been screenshot and re-uploaded hundreds of times with heavy distortion and glowing red eyes effect.',
      'Drakeposting': 'Generate a Drakeposting style meme from this photo. Create a two-panel format with the subject showing disapproval in the top panel and approval in the bottom panel. Apply the iconic Drake meme layout with clean panel division and expressive poses.',
      'Distracted BF': 'Generate a Distracted Boyfriend style meme from this photo. Apply the classic three-person jealousy meme format with labeled subjects. The composition should clearly show the looking-away dynamic with humorous text labels on each figure.',
      'Wojak': 'Generate a Wojak style meme from this photo. Transform the subject into a Wojak-inspired illustration with simple line art, minimal shading, and the characteristic emotional expression. Apply the distinctive MS Paint aesthetic with feel-guy styling.',
      'Cursed Image': 'Generate a cursed image style meme from this photo. Apply an unsettling, surreal quality with slightly off lighting, unusual angles, and an inexplicably uncomfortable atmosphere. The image should feel strange and humorous in a way that is hard to explain.',
      'Wholesome': 'Generate a wholesome meme from this photo. Apply warm, soft lighting and gentle color grading. Add heartwarming text with a friendly font. The overall feel should be uplifting, positive, and emotionally comforting with a gentle aesthetic.',
      'Comic Panel': 'Generate a comic panel style meme from this photo. Transform the image into a comic book panel with bold outlines, halftone dots, speech bubbles, and dramatic action lines. Apply a pop art or manga-inspired aesthetic with vibrant colors and comic typography.',
    };
    return stylePrompts[selectedMemeStyle.name] || `Generate a ${selectedMemeStyle.name} meme from this photo.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedMemeStyle ? { memeStyle: selectedMemeStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Meme generated successfully');
        ...
        if (statusData.status === 'failed') {
          setError(statusData.error || 'Failed to generate meme. Please try again.');
          return;
        }
      }
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {memeStylePresets.map((preset) => {
                      const isSelected = selectedMemeStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedMemeStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedMemeStyle
                        ? t('input.preset.summary.selected', { name: selectedMemeStyle.name })
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-meme-generator.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎨', before: `${R2_BASE}/ai-meme-generator/cases/case-1-before.png`, after: `${R2_BASE}/ai-meme-generator/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-meme-generator/cases/case-2-before.png`, after: `${R2_BASE}/ai-meme-generator/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-meme-generator/cases/case-3-before.png`, after: `${R2_BASE}/ai-meme-generator/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Meme Generator Preview"
          />
        </>
      )}
      <LoginModal
