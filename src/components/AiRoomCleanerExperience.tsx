...
export interface CleaningLevelPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiRoomCleanerExperienceProps {
  cleaningLevelPresets: CleaningLevelPresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/feature/after.png';

const CLEANING_PROMPT_MAP: Record<string, string> = {
  'Light Tidy': 'remove only loose surface clutter — papers, dishes, cups, laundry, cables, small trash. Keep all furniture, decor, rugs, wall art, plants and major items exactly as they are. The room should look tidied but still lived-in',
  ...
  'Furniture Only': 'remove every decor item, wall art, rug, plant, curtain, lamp, cushion, blanket, and accessory. Keep only the structural furniture (bed frame, sofa, desk, table, chairs). Bare practical interior, neutral walls',
...
export default function AiRoomCleanerExperience({ cleaningLevelPresets }: AiRoomCleanerExperienceProps) {
  const t = useTranslations('aiRoomCleaner');
  ...
  const [selectedLevel, setSelectedLevel] = useState<CleaningLevelPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedLevel) {
      return 'Keep the room exactly as it is. No cleaning applied.';
    }
    const cleaningDetail = CLEANING_PROMPT_MAP[selectedLevel.name] || `apply a ${selectedLevel.name.toLowerCase()} cleaning to this room`;
    return `Clean this room: ${cleaningDetail}. CRITICAL constraints: preserve the exact same room architecture — walls, windows, doors, ceiling, floor, room dimensions, viewpoint, camera angle, and overall composition must stay perfectly identical. Do not change wall colors unless explicitly requested. The result must look like the exact same room photographed from the exact same angle, with only the specified items removed.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedLevel ? { cleaningLevel: selectedLevel.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate cleaned room photo.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Room cleaned successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate cleaned room photo. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Cleaning level preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:hidden">
                    {cleaningLevelPresets.map((preset) => {
                      const isSelected = selectedLevel?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedLevel(isSelected ? null : preset)}
                      ...
                      {cleaningLevelPresets.map((preset) => {
                        ...
                        return (
                          <button
                            ...
                            key={preset.referenceSrc}
                            onClick={() => setSelectedLevel(isSelected ? null : preset)}
                            className={`flex w-[120px] flex-shrink-0 flex-col items-center rounded-2xl px-2 pb-2 pt-2 transition border-2 ${
                              isSelected
                                ? 'border-[#F0A202] bg-[#FFF4CC] shadow-[0_10px_25px_rgba(240,162,2,0.25)]'
                                : 'border-transparent bg-white text-slate-500'
                            }`}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedLevel
                        ? t('input.preset.summary.selected', { name: selectedLevel.name })
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🏠', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/cases/case-1-after.png' },
              { key: 2, icon: '🪑', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-cleaner/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
                  ...
                  <div className="relative rounded-[28px] overflow-hidden border border-[#FFE7A1] bg-[#FFF3B2]/40 aspect-[4/3] flex items-center justify-center">
                    <div className="grid grid-cols-2 gap-2 p-3 w-full h-full">
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.before} alt="Before room cleaning" className="w-full h-full object-cover" />
                        <span className="absolute bottom-1 left-1 bg-black/50 text-white text-xs px-1.5 py-0.5 rounded">Before</span>
                      </div>
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.after} alt="After room cleaning" className="w-full h-full object-cover" />
