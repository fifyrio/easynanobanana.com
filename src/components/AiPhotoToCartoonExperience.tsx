...
export interface CartoonStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiPhotoToCartoonExperienceProps {
  cartoonStylePresets: CartoonStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-photo-to-cartoon/feature/before.png`;
const afterImage = `${R2_BASE}/ai-photo-to-cartoon/feature/after.png`;

export default function AiPhotoToCartoonExperience({ cartoonStylePresets }: AiPhotoToCartoonExperienceProps) {
  const t = useTranslations('aiPhotoToCartoon');
  ...
  const [selectedCartoonStyle, setSelectedCartoonStyle] = useState<CartoonStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedCartoonStyle) {
      return 'Transform this photo into a cartoon illustration. Apply a general cartoon art style while preserving the subject\'s identity and key features.';
    }
    return `Transform this photo into a ${selectedCartoonStyle.name} cartoon style illustration. The result should look like a high-quality ${selectedCartoonStyle.name} artwork. Preserve the subject's identity, key features, and composition while applying the cartoon transformation faithfully.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedCartoonStyle ? { cartoonStyle: selectedCartoonStyle.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate cartoon transformation.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Cartoon transformation applied successfully');
      ...
      if (error.name === 'TypeError' && error.message?.includes('fetch')) {
        ...
        setError('Failed to generate cartoon transformation. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="photo-to-cartoon-upload"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {cartoonStylePresets.map((preset) => {
                      const isSelected = selectedCartoonStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedCartoonStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedCartoonStyle
                        ? t('input.preset.summary.selected', { name: selectedCartoonStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI photo to cartoon preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-photo-to-cartoon.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎨', before: `${R2_BASE}/ai-photo-to-cartoon/cases/case-1-before.png`, after: `${R2_BASE}/ai-photo-to-cartoon/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-photo-to-cartoon/cases/case-2-before.png`, after: `${R2_BASE}/ai-photo-to-cartoon/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-photo-to-cartoon/cases/case-3-before.png`, after: `${R2_BASE}/ai-photo-to-cartoon/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Photo to Cartoon Preview"
          />
        </>
      )}
    </>
