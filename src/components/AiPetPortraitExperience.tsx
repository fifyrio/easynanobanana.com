...
export interface PetPortraitStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiPetPortraitExperienceProps {
  petPortraitStylePresets: PetPortraitStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-pet-portrait/feature/before.png`;
const afterImage = `${R2_BASE}/ai-pet-portrait/feature/after.png`;

export default function AiPetPortraitExperience({ petPortraitStylePresets }: AiPetPortraitExperienceProps) {
  const t = useTranslations('aiPetPortrait');
  ...
  const [selectedPetPortraitStyle, setSelectedPetPortraitStyle] = useState<PetPortraitStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedPetPortraitStyle) {
      return 'Keep the pet photo as-is.';
    }
    return `Transform this pet photo into a beautiful ${selectedPetPortraitStyle.name} style artistic portrait. Maintain the pet's distinctive features, markings, and personality while applying the ${selectedPetPortraitStyle.name} art style. The result should be a high-quality artistic portrait.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPetPortraitStyle ? { petPortraitStyle: selectedPetPortraitStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate pet portrait.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Pet portrait generated successfully');
      ...
      if (error.name === 'TypeError' && error.message?.includes('fetch')) {
        ...
        setError('Failed to generate pet portrait. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Pet portrait style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {petPortraitStylePresets.map((preset) => {
                      const isSelected = selectedPetPortraitStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedPetPortraitStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedPetPortraitStyle
                        ? t('input.preset.summary.selected', { name: selectedPetPortraitStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated pet portrait preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🐾', before: `${R2_BASE}/ai-pet-portrait/cases/case-1-before.png`, after: `${R2_BASE}/ai-pet-portrait/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-pet-portrait/cases/case-2-before.png`, after: `${R2_BASE}/ai-pet-portrait/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-pet-portrait/cases/case-3-before.png`, after: `${R2_BASE}/ai-pet-portrait/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
