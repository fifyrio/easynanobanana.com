...
export interface OutfitStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiOutfitChangeExperienceProps {
  outfitStylePresets: OutfitStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-outfit-change/feature/before.png`;
const afterImage = `${R2_BASE}/ai-outfit-change/feature/after.png`;

export default function AiOutfitChangeExperience({ outfitStylePresets }: AiOutfitChangeExperienceProps) {
  const t = useTranslations('aiOutfitChange');
  ...
  const [selectedOutfit, setSelectedOutfit] = useState<OutfitStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedOutfit) {
      return 'Keep the subject exactly as they are. No outfit transformation.';
    }
    return `Change the outfit of the subject to a realistic ${selectedOutfit.name} style. Dress them appropriately for this fashion style with matching clothing, accessories, and styling details. Keep their identity, pose, and background identical.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedOutfit ? { outfit: selectedOutfit.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate outfit preview.');
        }
        return;
      }
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate outfit preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {outfitStylePresets.map((preset) => {
                      const isSelected = selectedOutfit?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedOutfit(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedOutfit
                        ? t('input.preset.summary.selected', { name: selectedOutfit.name })
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '👗', before: `${R2_BASE}/ai-outfit-change/cases/case-1-before.png`, after: `${R2_BASE}/ai-outfit-change/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-outfit-change/cases/case-2-before.png`, after: `${R2_BASE}/ai-outfit-change/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-outfit-change/cases/case-3-before.png`, after: `${R2_BASE}/ai-outfit-change/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Outfit Change Preview"
          />
        </>
      )}
      <LoginModal
