...
export interface EyeStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiOpenEyesExperienceProps {
  eyeStylePresets: EyeStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-open-eyes/feature/before.png`;
const afterImage = `${R2_BASE}/ai-open-eyes/feature/after.png`;

export default function AiOpenEyesExperience({ eyeStylePresets }: AiOpenEyesExperienceProps) {
  const t = useTranslations('aiOpenEyes');
  ...
  const [selectedEyeStyle, setSelectedEyeStyle] = useState<EyeStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedEyeStyle) {
      return 'Keep the subject\'s eyes exactly as they are. No eye modification.';
    }
    if (selectedEyeStyle.name === 'Natural Open') {
      return 'Make the subject\'s eyes appear naturally open and alert. Gently widen the eye aperture for a refreshed, awake look. The result should look effortless and authentic. Keep the same identity and features.';
    }
    return `Transform the subject's eyes to show a realistic ${selectedEyeStyle.name} look. The eye enhancement should appear natural and genuine, not artificial or over-edited. Keep their identity, clothing, pose, and background identical.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedEyeStyle ? { eyeStyle: selectedEyeStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate open eyes preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Eye enhancement applied successfully');
      ...
      if (error.name === 'TypeError' && error.message?.includes('fetch')) {
        ...
        setError('Failed to generate open eyes preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {eyeStylePresets.map((preset) => {
                      const isSelected = selectedEyeStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedEyeStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedEyeStyle
                        ? t('input.preset.summary.selected', { name: selectedEyeStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated open eyes preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '👁️', before: `${R2_BASE}/ai-open-eyes/cases/case-1-before.png`, after: `${R2_BASE}/ai-open-eyes/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-open-eyes/cases/case-2-before.png`, after: `${R2_BASE}/ai-open-eyes/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-open-eyes/cases/case-3-before.png`, after: `${R2_BASE}/ai-open-eyes/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
