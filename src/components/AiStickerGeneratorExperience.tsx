...
export interface StickerStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiStickerGeneratorExperienceProps {
  stickerStylePresets: StickerStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-sticker-generator/feature/before.png`;
const afterImage = `${R2_BASE}/ai-sticker-generator/feature/after.png`;

export default function AiStickerGeneratorExperience({ stickerStylePresets }: AiStickerGeneratorExperienceProps) {
  const t = useTranslations('aiStickerGenerator');
  ...
  const [selectedStickerStyle, setSelectedStickerStyle] = useState<StickerStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStickerStyle) {
      return 'Generate a creative sticker design from this photo, applying an artistic sticker style to the subject.';
    }
    const stylePrompts: Record<string, string> = {
      'Kawaii': 'Generate a kawaii sticker design from this photo. Create an adorable, cute Japanese-style sticker with big eyes, soft pastel colors, rounded shapes, and charming expressions. The design should feature chibi proportions, sparkles, hearts, and an overall sweet, lovable aesthetic with clean die-cut edges.',
      'Cartoon': 'Generate a cartoon sticker design from this photo. Create a bold, expressive cartoon-style sticker with vibrant colors, exaggerated features, thick outlines, and dynamic poses. The design should feature playful proportions, lively expressions, and a fun, animated aesthetic with clean sticker edges.',
      '3D Puffy': 'Generate a 3D puffy sticker design from this photo. Create a realistic three-dimensional puffy sticker with soft, inflated surfaces, glossy highlights, and smooth rounded edges. The design should look like a physical puffy sticker with depth, shadow effects, and a tactile, squeezable quality.',
      'Pixel Art': 'Generate a pixel art sticker design from this photo. Create a retro-style pixelated sticker with a limited color palette, clean pixel grid, and nostalgic 8-bit or 16-bit aesthetic. The design should feature crisp pixel edges, deliberate dithering patterns, and a charming retro gaming feel.',
      'Watercolor': 'Generate a watercolor sticker design from this photo. Create a soft, painterly sticker with watercolor washes, gentle color bleeds, and artistic brush strokes. The design should feature translucent layers, organic color transitions, and a delicate, hand-painted quality with defined sticker edges.',
      'Flat Design': 'Generate a flat design sticker from this photo. Create a modern, minimalist sticker with solid colors, clean geometric shapes, no gradients or shadows, and bold simplicity. The design should feature a contemporary flat illustration style with strong silhouettes and a polished, professional aesthetic.',
      'Doodle': 'Generate a doodle sticker design from this photo. Create a hand-drawn, sketchy sticker with playful line work, casual imperfections, and whimsical details. The design should look like it was drawn with a pen or marker, featuring loose lines, crosshatching, and a spontaneous, creative feel.',
      'Emoji': 'Generate an emoji-style sticker design from this photo. Create a round, expressive emoji sticker with bold, simple features, bright yellow or vibrant colors, and universally recognizable expressions. The design should feature clean, minimal details with maximum emotional impact in a circular format.',
    };
    return stylePrompts[selectedStickerStyle.name] || `Generate a ${selectedStickerStyle.name} sticker design from this photo.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStickerStyle ? { stickerStyle: selectedStickerStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate sticker design. Please try again.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Sticker design generated successfully');
        ...
        if (statusData.status === 'failed') {
          setError(statusData.error || 'Failed to generate sticker design. Please try again.');
          return;
        }
      }
      ...
      if (error.name === 'TypeError' && error.message?.includes('fetch')) {
        ...
        setError('Failed to generate sticker design. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {stickerStylePresets.map((preset) => {
                      const isSelected = selectedStickerStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedStickerStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedStickerStyle
                        ? t('input.preset.summary.selected', { name: selectedStickerStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated sticker design"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-sticker-generator.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎨', before: `${R2_BASE}/ai-sticker-generator/cases/case-1-before.png`, after: `${R2_BASE}/ai-sticker-generator/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-sticker-generator/cases/case-2-before.png`, after: `${R2_BASE}/ai-sticker-generator/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-sticker-generator/cases/case-3-before.png`, after: `${R2_BASE}/ai-sticker-generator/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Sticker Generator Preview"
          />
        </>
      )}
      <LoginModal
