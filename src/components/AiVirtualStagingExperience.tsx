...
export interface StagingStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiVirtualStagingExperienceProps {
  stylePresets: StagingStylePresetAsset[];
}

type RoomType = 'Living Room' | 'Bedroom' | 'Kitchen' | 'Bathroom' | 'Dining Room' | 'Office';

const ROOM_TYPES: RoomType[] = ['Living Room', 'Bedroom', 'Kitchen', 'Bathroom', 'Dining Room', 'Office'];
...
const STYLE_PROMPT_MAP: Record<string, string> = {
  ...
  'Luxury': 'a luxury style — plush velvet and leather furniture, rich jewel tones, marble and brass accents, crystal lighting, layered textures, designer decor, opulent finishes',
  'Minimalist': 'a minimalist style — ultra-clean lines, monochrome white and gray palette, very few carefully chosen pieces, hidden storage, abundant negative space, calm composition',
  'Farmhouse': 'a modern farmhouse style — shiplap accents, reclaimed wood furniture, warm neutral palette, cozy textiles, vintage-inspired decor, wrought iron fixtures, comfortable rustic charm',
  'Scandinavian': 'a Scandinavian style — light wood floors, white walls, soft neutral textiles, simple functional furniture, cozy throws, indoor plants, abundant natural light, hygge feel',
  'Industrial': 'an industrial style — exposed brick and metal, raw concrete, leather and steel furniture, Edison-bulb lighting, distressed wood, utilitarian decor, loft aesthetic',
  'Contemporary': 'a contemporary style — warm neutral tones, curved soft seating, layered textures, designer lighting, sculptural accents, refined collaborative composition',
  'Coastal': 'a coastal style — airy whites and soft blues, natural rattan and light wood, linen textiles, nautical accents, abundant natural light, relaxed beach-house atmosphere',
...
const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/feature/after.png';

export default function AiVirtualStagingExperience({ stylePresets }: AiVirtualStagingExperienceProps) {
  const t = useTranslations('aiVirtualStaging');
  ...
  const [selectedStyle, setSelectedStyle] = useState<StagingStylePresetAsset | null>(stylePresets[0] ?? null);
  ...
  const buildPrompt = () => {
    ...
    if (!style) {
      return `Virtually stage this empty ${roomType.toLowerCase()} by adding tasteful furniture and decor while preserving the architecture.`;
    }
    const styleDetail = STYLE_PROMPT_MAP[style.name] || `a ${style.name.toLowerCase()} interior style`;
    return `Virtually stage this empty ${roomType.toLowerCase()} in ${styleDetail}. Add realistic, well-arranged furniture, rugs, lighting, wall art, plants and decor appropriate for a ${roomType.toLowerCase()}, professionally styled for a real-estate listing. CRITICAL constraints: preserve the exact same room architecture — walls, windows, doors, ceiling, flooring, room dimensions, viewpoint and camera angle must stay perfectly identical. Do not move or alter any structural element. The result must look like the exact same physical room photographed from the exact same angle, now beautifully furnished in the chosen style.`;
  };
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate staged room.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Room staged successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate staged room. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="virtual-staging-upload"
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI virtually staged room preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-virtual-staging.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🛋️', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/cases/case-1-after.png' },
              { key: 2, icon: '🏠', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-virtual-staging/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
                  ...
                  <div className="relative rounded-[28px] overflow-hidden border border-[#FFE7A1] bg-[#FFF3B2]/40 aspect-[4/3] flex items-center justify-center">
                    <div className="grid grid-cols-2 gap-2 p-3 w-full h-full">
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.before} alt="Before staging" className="w-full h-full object-cover" />
                        <span className="absolute bottom-1 left-1 bg-black/50 text-white text-xs px-1.5 py-0.5 rounded">Before</span>
                      </div>
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.after} alt="After staging" className="w-full h-full object-cover" />
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Virtually Staged Room Preview"
          />
        </>
      )}
      <LoginModal
