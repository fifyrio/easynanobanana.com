...
interface AiFaceAnimatorExperienceProps {
  expressionPresets: ExpressionPresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-face-animator/feature/before.png`;
const afterImage = `${R2_BASE}/ai-face-animator/feature/after.png`;

export default function AiFaceAnimatorExperience({ expressionPresets }: AiFaceAnimatorExperienceProps) {
  const t = useTranslations('aiFaceAnimator');
  ...
  const buildPrompt = () => {
    if (!selectedExpression) {
      return 'Keep the subject exactly as they are. No facial expression change.';
    }
    return `Animate the face of the subject to show a "${selectedExpression.name}" expression. Make the expression look natural and realistic. Keep their identity, pose, clothing, and background identical.`;
  };
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate face animation.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Face animation applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate face animation. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated face animation preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '😄', before: `${R2_BASE}/ai-face-animator/cases/case-1-before.png`, after: `${R2_BASE}/ai-face-animator/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-face-animator/cases/case-2-before.png`, after: `${R2_BASE}/ai-face-animator/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-face-animator/cases/case-3-before.png`, after: `${R2_BASE}/ai-face-animator/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Face Animator Preview"
          />
        </>
      )}
      <LoginModal
