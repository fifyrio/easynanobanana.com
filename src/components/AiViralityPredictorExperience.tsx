...
export interface ViralityStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiViralityPredictorExperienceProps {
  viralityStylePresets: ViralityStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-virality-predictor/feature/before.png`;
const afterImage = `${R2_BASE}/ai-virality-predictor/feature/after.png`;

export default function AiViralityPredictorExperience({ viralityStylePresets }: AiViralityPredictorExperienceProps) {
  const t = useTranslations('aiViralityPredictor');
  ...
  const [selectedStyle, setSelectedStyle] = useState<ViralityStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Analyze and optimize the image for maximum social media virality. Enhance visual appeal, contrast, and engagement factors.';
    }
    return `Optimize this image for maximum virality on ${selectedStyle.name}. Apply platform-specific visual enhancements including optimal composition, color grading, contrast, brightness, and engagement-boosting elements that match the ${selectedStyle.name} aesthetic. Keep the subject and core content identical.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStyle ? { viralityStyle: selectedStyle.name } : undefined,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate virality preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Virality optimization applied successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate virality preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="virality-predictor-upload"
                ...
                {/* Virality style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {viralityStylePresets.map((preset) => {
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI virality-optimized preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-virality-predictor.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '📈', before: `${R2_BASE}/ai-virality-predictor/cases/case-1-before.png`, after: `${R2_BASE}/ai-virality-predictor/cases/case-1-after.png` },
              { key: 2, icon: '🔥', before: `${R2_BASE}/ai-virality-predictor/cases/case-2-before.png`, after: `${R2_BASE}/ai-virality-predictor/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-virality-predictor/cases/case-3-before.png`, after: `${R2_BASE}/ai-virality-predictor/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Virality Predictor Preview"
          />
        </>
      )}
      <LoginModal
