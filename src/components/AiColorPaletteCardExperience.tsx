...
export interface ColorPaletteCardPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiColorPaletteCardExperienceProps {
  cardPresets: ColorPaletteCardPresetAsset[];
}

const PROMPT_MAP: Record<string, string> = {
  'Split Cover': 'Design an aesthetic color palette card in a split-cover layout. Place the original photo on one half and a stacked column of extracted dominant color swatches with their HEX codes on the other half. Clean editorial spacing, soft drop shadows, refined sans-serif typography. A premium social-media-ready memory card.',
  'Palette Diary': 'Design an aesthetic color palette card in a journal/diary layout. Show the photo as a framed snapshot, a horizontal row of 5 extracted dominant color swatches with HEX labels underneath, and tasteful handwritten-style captions. Warm paper texture, scrapbook feel, cozy nostalgic mood.',
  'Moment Poster': 'Design an aesthetic color palette card as a minimalist art poster. Large photo as the hero, a slim strip of extracted dominant colors along the bottom edge, bold poster-style title typography and small metadata. Gallery-grade composition with generous negative space.',
  'Film Strip': 'Design an aesthetic color palette card styled like analog film. Present the photo inside a film frame with sprocket holes, a vertical filmstrip of extracted dominant color swatches beside it, and retro film-stock labels with HEX codes. Grainy, cinematic, vintage aesthetic.',
  'Minimal Swatch': 'Design an aesthetic color palette card with an ultra-minimal Swiss layout. A neat grid of extracted dominant color swatches with precise HEX codes, the photo as a small contained thumbnail, lots of white space, crisp grid alignment, modern premium typography.',
  'Magazine': 'Design an aesthetic color palette card as a glossy magazine spread. Full-bleed photo, an elegant overlaid color palette bar of extracted dominant tones with HEX codes, editorial headline typography, column accents and page metadata. High-fashion print look.',
  'Polaroid Stack': 'Design an aesthetic color palette card as a stack of polaroid photos. Show the main photo as the top polaroid with a handwritten caption on the white border, plus extracted dominant color swatches arranged as small tiles around it. Playful, casual, instant-camera mood.',
  'Gradient Mood': 'Design an aesthetic color palette card with a smooth gradient mood-board background blended from the extracted dominant colors of the photo. Float the photo as a rounded card on top, list the dominant HEX codes elegantly, dreamy atmospheric ambient style.',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/feature/after.png';

export default function AiColorPaletteCardExperience({ cardPresets }: AiColorPaletteCardExperienceProps) {
  const t = useTranslations('aiColorPaletteCard');
  ...
  const [selectedPreset, setSelectedPreset] = useState<ColorPaletteCardPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    const titleInstruction = cardTitle.trim()
      ? ` The card title text is "${cardTitle.trim()}". Display this title elegantly on the card with refined, readable typography that complements the layout.`
      : '';
    if (!selectedPreset) {
      return `Turn this photo into a beautiful aesthetic color palette card. Extract the dominant colors from the photo and present them as a row of swatches with HEX codes alongside the photo in a clean, share-ready design.${titleInstruction}`;
    }
    const base = PROMPT_MAP[selectedPreset.name] || `Turn this photo into a ${selectedPreset.name} style aesthetic color palette card with extracted dominant color swatches and HEX codes.`;
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPreset ? { cardStyle: selectedPreset.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate palette card.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Color palette card generated successfully');
            await refreshProfile();
            return;
          } else {
            setError('Card generation completed but no result URL found.');
            return;
          }
        }

        if (statusData.status === 'failed') {
          setError(statusData.error || 'Card generation failed. Please try again.');
          return;
        }
      }

      setError('Card generation is taking longer than expected. Please check back later.');
    } catch (err: unknown) {
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate palette card. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="color-palette-card-upload"
                ...
                {/* Card title input */}
                <div>
                  <label htmlFor="card-title" className="block text-sm font-semibold text-slate-900 mb-2">
                    {t('input.cardTitle.label')}
                  </label>
                  <input
                    type="text"
                    id="card-title"
                    ...
                    placeholder={t('input.cardTitle.placeholder')}
                    maxLength={100}
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">{t('input.cardTitle.hint')}</p>
                </div>
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated color palette card preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-color-palette-card.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/cases/case-1-after.png' },
              { key: 2, icon: '🖼️', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-color-palette-card/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Color Palette Card Preview"
          />
        </>
      )}
      <LoginModal
