...
export interface YearbookStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiYearbookExperienceProps {
  yearbookStylePresets: YearbookStylePresetAsset[];
}

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/feature/after.png';

export default function AiYearbookExperience({ yearbookStylePresets }: AiYearbookExperienceProps) {
  const t = useTranslations('aiYearbook');
  ...
  const [selectedYearbookStyle, setSelectedYearbookStyle] = useState<YearbookStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedYearbookStyle) {
      return 'Transform this photo into a generic retro yearbook portrait. Vintage school photo aesthetic, soft lighting, faded colors, classic backdrop.';
    }
    const styleMap: Record<string, string> = {
      '90s Classic': 'Transform this person into an authentic 1990s high school yearbook portrait. Feathered or layered hair, oversized collar or denim jacket, soft studio lighting, blue laser gradient backdrop. Slight grain, washed-out 90s photo color tone. Keep the person\'s identity and face recognizable.',
      '80s Glam': 'Transform this person into an authentic 1980s high school yearbook portrait. Big voluminous teased curly hair, bright pastel or neon backdrop, shoulder-pad blazer or sequined top, soft glam makeup. Vibrant 80s color palette with slight grain. Keep the person\'s identity and face recognizable.',
      '70s Disco': 'Transform this person into an authentic 1970s high school yearbook portrait. Long center-parted hair or afro, wide bell-shaped collar with bold patterns, warm earth-tone backdrop. Faded 70s film color, slight grain. Keep the person\'s identity and face recognizable.',
      'Prom Queen': 'Transform this person into a 1990s prom queen yearbook portrait. Elegant updo hairstyle, sparkling tiara, formal satin gown with sweetheart neckline, soft glowing backlight, blue gradient backdrop. Soft romantic yearbook aesthetic. Keep the person\'s identity and face recognizable.',
      'Class Nerd': 'Transform this person into a 1990s class nerd yearbook portrait. Large round wire-frame glasses, tightly buttoned plaid shirt with sweater vest or bow tie, neatly combed hair, awkward warm smile, blue gradient backdrop. Classic yearbook lighting. Keep the person\'s identity and face recognizable.',
      'Cheerleader': 'Transform this person into a 1990s high school cheerleader yearbook portrait. Wearing a varsity cheerleader uniform with school colors (red and white), holding pom-poms, hair tied with a ribbon, bright energetic smile, blue gradient backdrop. Classic yearbook lighting. Keep the person\'s identity and face recognizable.',
      'Star Athlete': 'Transform this person into a 1990s high school star athlete yearbook portrait. Wearing a varsity letterman jacket with school patch, slight tousled hair, confident smile, blue or gymnasium backdrop. Classic yearbook studio lighting. Keep the person\'s identity and face recognizable.',
      'Goth Kid': 'Transform this person into a 1990s high school goth yearbook portrait. Dyed black hair with straight bangs, dark eyeliner and lipstick, black mesh or velvet top with silver chain choker, pale skin, brooding expression, dark gradient backdrop. Classic yearbook composition. Keep the person\'s identity and face recognizable.',
      'Punk Rocker': 'Transform this person into a 1990s high school punk rocker yearbook portrait. Spiked or mohawk-style hair (possibly dyed bright color), leather jacket with band patches and studs, ripped t-shirt visible underneath, confident smirk, gritty backdrop. Classic yearbook lighting with edge. Keep the person\'s identity and face recognizable.',
    };
    return styleMap[selectedYearbookStyle.name] || `Transform this person into an authentic retro ${selectedYearbookStyle.name} yearbook portrait. Vintage school photo aesthetic, classic studio backdrop. Keep the person's identity and face recognizable.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedYearbookStyle ? { yearbookStyle: selectedYearbookStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate yearbook portrait.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Yearbook portrait generated successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate yearbook portrait. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Yearbook style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {yearbookStylePresets.map((preset) => {
                      const isSelected = selectedYearbookStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedYearbookStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedYearbookStyle
                        ? t('input.preset.summary.selected', { name: selectedYearbookStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated yearbook portrait preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-yearbook-portrait.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎓', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/cases/case-1-after.png' },
              { key: 2, icon: '📚', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-yearbook-generator/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
                  ...
                  <div className="relative rounded-[28px] overflow-hidden border border-[#FFE7A1] bg-[#FFF3B2]/40 aspect-[4/3] flex items-center justify-center">
                    <div className="grid grid-cols-2 gap-2 p-3 w-full h-full">
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.before} alt="Before yearbook style" className="w-full h-full object-cover" />
                        <span className="absolute bottom-1 left-1 bg-black/50 text-white text-xs px-1.5 py-0.5 rounded">Before</span>
                      </div>
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.after} alt="After yearbook style" className="w-full h-full object-cover" />
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Yearbook Portrait Preview"
          />
        </>
      )}
      <LoginModal
