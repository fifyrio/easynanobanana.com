...
const beforeImage = `${R2_BASE}/ai-face-swap/feature/before.png`;
const afterImage = `${R2_BASE}/ai-face-swap/feature/after.png`;

export default function AiFaceSwapExperience() {
  ...
  const buildPrompt = () => {
    return 'Using the 2 uploaded photos, perform a high-quality face swap: take the face from the first image (source face) and place it onto the person in the second image (target). Preserve the target image\'s body, pose, hair, clothing, background, lighting, and composition. Match the source face\'s identity, expression, and natural skin tone while seamlessly blending with the target\'s lighting and shadows. Maintain the target image\'s original aspect ratio and resolution. Deliver a photorealistic face swap result.';
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: { faceCount: imageUrls.length, tool: 'face-swap' },
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate face swap.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('AI face swap generated successfully');
      ...
      if (err instanceof Error) {
        if (err.name === 'TypeError' && err.message.includes('fetch')) {
          ...
          setError(err.message || 'Failed to generate face swap.');
        }
      } else {
        setError('Failed to generate face swap.');
      }
    } finally {
  ...
  // For preview: show target image (slot 2) as 'before', generated as 'after'
  const previewTargetImage = slots[1]?.preview || slots[1]?.url || beforeImage;
  const beforeDisplayImage = previewTargetImage;
  const afterDisplayImage = generatedImage || afterImage;
  const beforeTag = slots[1]?.preview || slots[1]?.url ? t('preview.labels.original') : t('preview.labels.before');
  ...
  const slotLabelKey = (index: number): string => {
    ...
    return 'input.upload.slots.target';
  };
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
                ...
                <div className="rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-5 shadow-inner space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {slots.map((slot, index) => {
                      const inputId = `face-swap-upload-${index}`;
                      ...
                      return (
                        <div key={index} className="space-y-1.5">
                          <div className="flex items-center justify-between text-[11px] font-semibold text-slate-700">
                            ...
                            <span className="text-[10px] uppercase tracking-wide text-[#C69312]">
              ...
              <div className="grid grid-cols-2 gap-3 pt-2 text-center">
                <div className="rounded-2xl border border-[#FFE7A1] bg-white/70 px-2 py-3">
                  <div className="text-lg font-semibold text-slate-900">{t('input.stats.quality.value')}</div>
                  <div className="text-[11px] uppercase tracking-wide text-slate-500">{t('input.stats.quality.label')}</div>
                </div>
                <div className="rounded-2xl border border-[#FFE7A1] bg-white/70 px-2 py-3">
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated face swap preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🔀', before: `${R2_BASE}/ai-face-swap/cases/case-1-before.png`, after: `${R2_BASE}/ai-face-swap/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-face-swap/cases/case-2-before.png`, after: `${R2_BASE}/ai-face-swap/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-face-swap/cases/case-3-before.png`, after: `${R2_BASE}/ai-face-swap/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
