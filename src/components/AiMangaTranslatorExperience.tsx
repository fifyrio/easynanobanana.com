...
export interface MangaTranslatorPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiMangaTranslatorExperienceProps {
  translationPresets: MangaTranslatorPresetAsset[];
}

const PROMPT_MAP: Record<string, string> = {
  'English': 'Translate all text in this manga/comic page into English. Replace all dialogue in speech bubbles, narration boxes, sound effects (onomatopoeia), and any visible text with accurate English translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. The translated text should fit naturally within the existing bubbles and text areas. Use appropriate manga reading conventions.',
  'Chinese Simplified': 'Translate all text in this manga/comic page into Simplified Chinese (简体中文). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate Simplified Chinese translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural Chinese expressions and proper simplified characters.',
  'Chinese Traditional': 'Translate all text in this manga/comic page into Traditional Chinese (繁體中文). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate Traditional Chinese translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural Chinese expressions and proper traditional characters.',
  'Japanese': 'Translate all text in this manga/comic page into Japanese (日本語). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate Japanese translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use appropriate kanji, hiragana, and katakana. Maintain manga conventions.',
  'Korean': 'Translate all text in this manga/comic page into Korean (한국어). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate Korean translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural Korean expressions and proper Hangul.',
  'French': 'Translate all text in this manga/comic page into French (Français). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate French translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural French expressions appropriate for manga/BD format.',
  'Spanish': 'Translate all text in this manga/comic page into Spanish (Español). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate Spanish translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural Spanish expressions.',
  'German': 'Translate all text in this manga/comic page into German (Deutsch). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate German translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural German expressions.',
  'Portuguese': 'Translate all text in this manga/comic page into Portuguese (Português). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate Portuguese translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural Portuguese expressions.',
  'Russian': 'Translate all text in this manga/comic page into Russian (Русский). Replace all dialogue in speech bubbles, narration boxes, sound effects, and any visible text with accurate Russian translations. Preserve the original panel layout, speech bubble shapes, character art, and visual style exactly. Use natural Russian expressions and Cyrillic script.',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/feature/after.png';

export default function AiMangaTranslatorExperience({ translationPresets }: AiMangaTranslatorExperienceProps) {
  const t = useTranslations('aiMangaTranslator');
  ...
  const [selectedPreset, setSelectedPreset] = useState<MangaTranslatorPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    const customInstruction = translationInstruction.trim()
      ? ` Additional translation instructions: ${translationInstruction.trim()}`
      : '';
    if (!selectedPreset) {
      return `Translate all text in this manga/comic page into English. Preserve the original panel layout, speech bubbles, character art, and visual style exactly. Replace dialogue, narration, sound effects, and any visible text with accurate translations.${customInstruction}`;
    }
    const base = PROMPT_MAP[selectedPreset.name] || `Translate all text in this manga/comic page into ${selectedPreset.name}. Preserve the original panel layout, speech bubbles, character art, and visual style exactly.`;
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPreset ? { targetLanguage: selectedPreset.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to translate manga.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Manga translated successfully');
            await refreshProfile();
            return;
          } else {
            setError('Translation completed but no result URL found.');
            return;
          }
        }

        if (statusData.status === 'failed') {
          setError(statusData.error || 'Translation failed. Please try again.');
          return;
        }
      }
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to translate manga. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="manga-translator-upload"
                ...
                {/* Translation instruction input */}
                <div>
                  <label htmlFor="translation-instruction" className="block text-sm font-semibold text-slate-900 mb-2">
                    {t('input.instruction.label')}
                  </label>
                  <textarea
                    id="translation-instruction"
                    value={translationInstruction}
                    onChange={(e) => setTranslationInstruction(e.target.value)}
                    placeholder={t('input.instruction.placeholder')}
                    maxLength={300}
                    rows={3}
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">{t('input.instruction.hint')}</p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      { value: '3:4', w: 'w-3', h: 'h-4' },
                      { value: '1:1', w: 'w-4', h: 'h-4' },
                      { value: '4:3', w: 'w-4', h: 'h-3' },
                ...
                {/* Target language preset selector */}
                <div>
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI translated manga preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-manga-translated.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '📖', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/cases/case-1-after.png' },
              { key: 2, icon: '🌍', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-manga-translator/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Manga Translation Preview"
          />
        </>
      )}
      <LoginModal
