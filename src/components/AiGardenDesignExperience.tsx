...
export interface GardenStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiGardenDesignExperienceProps {
  stylePresets: GardenStylePresetAsset[];
}

type GardenType = 'Backyard' | 'Front Yard' | 'Balcony' | 'Patio' | 'Terrace' | 'Courtyard';

const GARDEN_TYPES: GardenType[] = ['Backyard', 'Front Yard', 'Balcony', 'Patio', 'Terrace', 'Courtyard'];

const GARDEN_TYPE_I18N: Record<GardenType, string> = {
  'Backyard': 'backyard',
...
const STYLE_PROMPT_MAP: Record<string, string> = {
  'Modern': 'a modern garden design — clean geometric lines, sleek concrete and corten steel planters, architectural plants, manicured lawn strips, minimalist water feature, ambient outdoor lighting',
  'Mediterranean': 'a Mediterranean garden design — terracotta pots, gravel paths, olive and citrus trees, lavender and rosemary, stone walls, a tiled fountain, warm earthy palette, pergola with climbing vines',
  'English': 'an English cottage garden design — lush layered flower borders, roses and foxgloves, winding stone paths, trimmed hedges, a classic arbor, abundant blooms in soft romantic colors',
  'Japanese': 'a Japanese Zen garden design — raked gravel, moss, sculptural stones, a stone lantern, a koi pond, maple trees, bamboo, pruned evergreens, calm meditative atmosphere',
  'Tropical': 'a tropical garden design — large lush palms, banana plants, vibrant exotic flowers, dense green foliage, natural stone, a water feature, bamboo accents, resort-like atmosphere',
  'Cottage': 'a cottage garden design — informal dense plantings, mixed perennials and herbs, climbing roses, rustic wooden fences, a charming gravel path, wildflowers, a relaxed homey feel',
  'Minimalist': 'a minimalist garden design — restrained plant palette, clean lines, large negative space, a single sculptural tree, neat gravel or paving, monochrome planters, calm uncluttered composition',
  'Desert': 'a desert xeriscape garden design — succulents, agave and cacti, decorative gravel, boulders, drought-tolerant grasses, warm sandy tones, sculptural arid plants, water-wise layout',
  'Formal': 'a formal garden design — symmetrical layout, precisely clipped boxwood hedges and topiary, geometric parterres, a central fountain, gravel walkways, classical urns, manicured elegance',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/feature/after.png';

export default function AiGardenDesignExperience({ stylePresets }: AiGardenDesignExperienceProps) {
  const t = useTranslations('aiGardenDesign');
  ...
  const [selectedStyle, setSelectedStyle] = useState<GardenStylePresetAsset | null>(stylePresets[0] ?? null);
  const [gardenType, setGardenType] = useState<GardenType>('Backyard');
  ...
  const buildPrompt = () => {
    ...
    if (!style) {
      return `Redesign this ${gardenType.toLowerCase()} with new landscaping and planting while preserving the property boundaries.`;
    }
    const styleDetail = STYLE_PROMPT_MAP[style.name] || `a ${style.name.toLowerCase()} garden design`;
    return `Restyle this ${gardenType.toLowerCase()} into ${styleDetail}. Replace all plants, paving, lawn, decking, planters, outdoor furniture, lighting and landscaping features to match the chosen style. CRITICAL constraints: preserve the exact same property boundaries, surrounding buildings and walls, fences, ground footprint and dimensions, viewpoint and camera angle — they must stay perfectly identical. The result must look like the exact same physical outdoor space photographed from the exact same angle, but completely redesigned in the chosen garden style.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStyle ? { style: selectedStyle.name, gardenType } : { gardenType },
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate garden design.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Garden redesigned successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate garden design. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Garden type selector (chips) */}
                <div>
                  <p className="text-sm font-semibold text-slate-900 mb-2">{t('input.gardenType.label')}</p>
                  <div className="flex flex-wrap gap-2">
                    {GARDEN_TYPES.map((gt) => {
                      const isSelected = gardenType === gt;
                      return (
                        <button
                          key={gt}
                          type="button"
                          onClick={() => setGardenType(gt)}
                        ...
                        >
                          {t(`input.gardenType.options.${GARDEN_TYPE_I18N[gt]}`)}
                        </button>
                      );
                    })}
                  </div>
                    ...
                    <p>
                      {selectedStyle
                        ? t('input.preset.summary.selected', {
                            ...
                            gardenType: t(`input.gardenType.options.${GARDEN_TYPE_I18N[gardenType]}`),
                          })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI redesigned garden preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🪴', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/cases/case-1-after.png' },
              { key: 2, icon: '🌳', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/cases/case-2-after.png' },
              { key: 3, icon: '🌸', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-garden-design/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Redesigned Garden Preview"
          />
        </>
      )}
      <LoginModal
