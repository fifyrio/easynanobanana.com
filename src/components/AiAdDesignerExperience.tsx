...
export interface AdDesignerPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiAdDesignerExperienceProps {
  adPresets: AdDesignerPresetAsset[];
}

const PROMPT_MAP: Record<string, string> = {
  'Social Media Post': 'Transform this image into a professional social media post design. Create an eye-catching Instagram/Facebook post layout with the product/subject prominently featured, bold headline text overlay, engaging call-to-action button, brand-friendly color scheme, and modern social media aesthetic. Square 1:1 format optimized for feeds.',
  'Product Banner': 'Transform this image into a clean professional product banner advertisement. Feature the product/subject centered with ample white space, sleek modern typography for product name and tagline, subtle gradient background, price tag or feature highlights, and a polished e-commerce aesthetic. Wide horizontal banner format.',
  'YouTube Thumbnail': 'Transform this image into a bold attention-grabbing YouTube thumbnail. Create a high-contrast design with the subject enlarged, dramatic lighting effects, large bold text overlay with 2-3 impactful words, bright saturated colors (red, yellow, blue accents), expressive composition, and clickbait-worthy energy. 16:9 format.',
  'Story Ad': 'Transform this image into a vertical Instagram/TikTok story advertisement. Full-bleed vertical layout with the product/subject featured prominently, swipe-up CTA arrow, gradient text overlay, trendy Gen-Z aesthetic with bold sans-serif typography, motion-inspired design elements. 9:16 vertical format.',
  'Sale Banner': 'Transform this image into an urgent sale/promotion banner. Add a large bold "SALE" or "50% OFF" text, bright red/orange/yellow sale colors, starburst or badge elements, strikethrough original price, countdown urgency elements, and eye-catching promotional graphics. High-energy retail sale aesthetic.',
  'Luxury Brand': 'Transform this image into an elegant luxury brand advertisement. Apply a sophisticated dark or cream palette, gold or silver foil accents, refined serif typography, generous negative space, subtle texture (marble, leather grain), premium product presentation with shadow and reflection. High-end fashion/luxury aesthetic.',
  'Minimalist Ad': 'Transform this image into a clean minimalist advertisement design. Abundant white/light space, the product/subject isolated cleanly, thin elegant sans-serif typography, single accent color, precise geometric alignment, Scandinavian-inspired simplicity. Less is more — sophisticated restraint.',
  'Neon Promo': 'Transform this image into a vibrant neon-style promotional design. Electric neon glow effects (hot pink, cyan, electric purple), dark background, glowing text outlines, retro-futuristic typography, party/nightlife energy, light trail effects, and dynamic composition. Nightclub/festival promotional aesthetic.',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/feature/after.png';

export default function AiAdDesignerExperience({ adPresets }: AiAdDesignerExperienceProps) {
  const t = useTranslations('aiAdDesigner');
  ...
  const [selectedPreset, setSelectedPreset] = useState<AdDesignerPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedPreset) {
      return 'Transform this image into a professional marketing advertisement design. Create a polished, eye-catching ad with compelling composition and typography.';
    }
    return PROMPT_MAP[selectedPreset.name] || `Transform this image into a ${selectedPreset.name} style advertisement design. Create a professional, publish-ready ad with the ${selectedPreset.name} aesthetic.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPreset ? { adStyle: selectedPreset.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate ad design.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Ad design generated successfully');
            await refreshProfile();
            return;
          } else {
            setError('Design generation completed but no result URL found.');
            return;
          }
        }

        if (statusData.status === 'failed') {
          setError(statusData.error || 'Design generation failed. Please try again.');
          return;
        }
      }

      setError('Design generation is taking longer than expected. Please check back later.');
    } catch (err: any) {
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate ad design. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated ad design preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '📢', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/cases/case-1-after.png' },
              { key: 2, icon: '🎨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-ad-designer/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
