...
export interface GenderStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiGenderSwapExperienceProps {
  genderStylePresets: GenderStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-gender-swap/feature/before.png`;
const afterImage = `${R2_BASE}/ai-gender-swap/feature/after.png`;

export default function AiGenderSwapExperience({ genderStylePresets }: AiGenderSwapExperienceProps) {
  const t = useTranslations('aiGenderSwap');
  ...
  const [selectedStyle, setSelectedStyle] = useState<GenderStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedStyle) {
      return 'Transform the gender presentation of this person to appear as the opposite gender. Apply natural-looking changes to facial features, hair, and skin texture while preserving the person\'s identity, pose, clothing style, and background.';
    }

    const prompts: Record<string, string> = {
      'Female Natural': 'Transform this person to appear as a natural-looking woman. Soften the jawline, add subtle feminine facial features, adjust eyebrows to a feminine arch, add natural-looking longer hair or feminine hairstyle, smooth skin texture slightly, and add very subtle natural makeup. The result should look like a real woman — not exaggerated or artificial. Preserve identity, pose, and background.',
      'Male Natural': 'Transform this person to appear as a natural-looking man. Strengthen the jawline, add masculine facial features, thicken eyebrows slightly, add a masculine short hairstyle, add subtle facial hair shadow or light stubble, and give skin a slightly more textured appearance. The result should look like a real man — not exaggerated. Preserve identity, pose, and background.',
      'Androgynous': 'Transform this person to appear androgynous and gender-neutral. Balance feminine and masculine features equally — soften harsh angles but maintain some structure, create a gender-ambiguous hairstyle, even out skin texture, and create features that could read as either gender. The result should be beautifully balanced between masculine and feminine. Preserve identity, pose, and background.',
      'Soft Feminine': 'Apply a subtle feminization to this person. Gently soften facial features, slightly refine the jawline, add a hint of feminine eyebrow shaping, create softer skin texture, and add very minimal natural-looking changes. This should be a gentle shift — noticeable but not dramatic. Preserve identity, pose, clothing, and background.',
      'Strong Masculine': 'Apply a strong masculinization to this person. Significantly strengthen the jawline, add prominent masculine brow ridge, thicken eyebrows, add visible stubble or short beard, create a masculine short haircut, and add more defined facial structure. The result should look distinctly masculine and strong. Preserve identity, pose, and background.',
      'Glamorous Female': 'Transform this person to appear as a glamorous woman. Add feminine features with professional-quality makeup — smokey eyes, defined brows, contoured cheekbones, full lips with lipstick, and an elegant feminine hairstyle. Smooth and luminous skin. The result should look like a red-carpet-ready glamorous woman. Preserve identity, pose, and background.',
      'Rugged Male': 'Transform this person to appear as a rugged, masculine man. Add a strong square jaw, visible stubble or short beard, weathered slightly textured skin, thick masculine eyebrows, short tousled hair, and prominent cheekbones. The result should look like a rugged, outdoorsy masculine man. Preserve identity, pose, and background.',
      'Elegant Female': 'Transform this person to appear as an elegant, refined woman. Add graceful feminine features, a well-defined but soft jawline, elegantly shaped eyebrows, a sophisticated hairstyle (like a sleek updo or soft waves), flawless skin with minimal natural makeup, and refined bone structure. The result should look poised and elegant. Preserve identity, pose, and background.',
    };

    return prompts[selectedStyle.name] || prompts['Female Natural'];
  };
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate gender swap result.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Gender swap completed successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate gender swap result. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🔄', before: `${R2_BASE}/ai-gender-swap/cases/case-1-before.png`, after: `${R2_BASE}/ai-gender-swap/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-gender-swap/cases/case-2-before.png`, after: `${R2_BASE}/ai-gender-swap/cases/case-2-after.png` },
              { key: 3, icon: '🎭', before: `${R2_BASE}/ai-gender-swap/cases/case-3-before.png`, after: `${R2_BASE}/ai-gender-swap/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
