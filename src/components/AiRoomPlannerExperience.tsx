...
export interface StylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiRoomPlannerExperienceProps {
  stylePresets: StylePresetAsset[];
}

type RoomType = 'Living Room' | 'Bedroom' | 'Dining Room' | 'Kitchen' | 'Bathroom' | 'Office';

const ROOM_TYPES: RoomType[] = ['Living Room', 'Bedroom', 'Dining Room', 'Kitchen', 'Bathroom', 'Office'];

const ROOM_TYPE_I18N: Record<RoomType, string> = {
...
const STYLE_PROMPT_MAP: Record<string, string> = {
  'Modern': 'a contemporary modern interior design — clean lines, neutral palette with bold accents, polished surfaces, sleek minimal furniture, statement lighting, large-scale art',
  'Scandinavian': 'a Scandinavian interior design — light wood floors, white walls, soft neutral textiles, simple functional furniture, cozy throws, indoor plants, abundant natural light',
  'Industrial': 'an industrial interior design — exposed brick walls, metal framing, raw concrete floors, Edison-bulb pendant lighting, leather and metal furniture, distressed wood',
  'Minimalist': 'a minimalist interior design — ultra-clean lines, monochrome white and gray palette, very few items, hidden storage, negative space, single sculptural focal piece',
  'Bohemian': 'a bohemian interior design — layered colorful rugs and textiles, macramé wall hangings, rattan furniture, lush plants, eclectic global decor, warm earthy tones',
  'Japandi': 'a Japandi interior design — blending Japanese and Scandinavian aesthetics — natural wood, muted neutral palette, low-profile furniture, paper lanterns, zen plants, clean uncluttered lines',
  'Mediterranean': 'a Mediterranean interior design — terracotta tile floors, white plaster walls, arched doorways, wrought iron details, warm earthy palette, ceramic pottery, olive plants',
  'Vintage': 'a vintage interior design — mid-century furniture, retro patterned wallpaper, brass accents, antique decor, warm amber tones, classic floor lamps, framed art prints',
  'Traditional': 'a traditional interior design — classic crown moldings, ornate wood furniture, rich fabric upholstery, oriental rugs, symmetrical layouts, formal drapery, framed oil paintings',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/feature/after.png';

export default function AiRoomPlannerExperience({ stylePresets }: AiRoomPlannerExperienceProps) {
  const t = useTranslations('aiRoomPlanner');
  ...
  const [selectedStyle, setSelectedStyle] = useState<StylePresetAsset | null>(stylePresets[0] ?? null);
  ...
  const buildPrompt = () => {
    ...
    if (!style) {
      return `Redecorate this ${roomType.toLowerCase()} with new furniture and decor while preserving the architecture.`;
    }
    const styleDetail = STYLE_PROMPT_MAP[style.name] || `a ${style.name.toLowerCase()} interior design`;
    return `Restyle this ${roomType.toLowerCase()} into ${styleDetail}. Replace all furniture, decor, lighting, rugs, wall art, and surface materials to match the chosen style. CRITICAL constraints: preserve the exact same room architecture — walls, windows, doors, ceiling, floor layout, room dimensions, viewpoint and camera angle must stay perfectly identical. The result must look like the exact same physical room photographed from the exact same angle, but completely redecorated in the chosen style.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedStyle ? { style: selectedStyle.name, roomType } : { roomType },
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate redesigned room.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Room redesigned successfully');
      ...
      if (err.name === 'TypeError' && err.message.includes('fetch')) {
        ...
        setError('Failed to generate redesigned room. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900 mb-2">{t('input.roomType.label')}</p>
                  <div className="flex flex-wrap gap-2">
                    {ROOM_TYPES.map((rt) => {
                      ...
                      return (
                        ...
                        >
                          {t(`input.roomType.options.${ROOM_TYPE_I18N[rt]}`)}
                        </button>
                      );
                    })}
                  </div>
                    ...
                    <p>
                      {selectedStyle
                        ? t('input.preset.summary.selected', {
                            ...
                            roomType: t(`input.roomType.options.${ROOM_TYPE_I18N[roomType]}`),
                          })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI redesigned room preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🛋️', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/cases/case-1-after.png' },
              { key: 2, icon: '🏡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-room-planner/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Redesigned Room Preview"
          />
        </>
      )}
      <LoginModal
