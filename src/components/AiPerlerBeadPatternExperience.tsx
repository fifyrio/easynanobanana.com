...
export interface PerlerBeadStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiPerlerBeadPatternExperienceProps {
  perlerBeadStylePresets: PerlerBeadStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-perler-bead-pattern/feature/before.png`;
const afterImage = `${R2_BASE}/ai-perler-bead-pattern/feature/after.png`;

export default function AiPerlerBeadPatternExperience({ perlerBeadStylePresets }: AiPerlerBeadPatternExperienceProps) {
  const t = useTranslations('aiPerlerBeadPattern');
  ...
  const [selectedPerlerBeadStyle, setSelectedPerlerBeadStyle] = useState<PerlerBeadStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedPerlerBeadStyle) {
      return 'Convert this photo into a perler bead pixel art pattern with a visible grid of circular beads.';
    }
    const stylePrompts: Record<string, string> = {
      ...
      'Mini Beads': 'Convert this photo into a high-resolution mini perler bead pattern. Use very small, fine-detail beads for maximum image resolution and detail. Tiny circular beads in a dense grid pattern, allowing for smoother color transitions and finer details than standard beads. Realistic mini bead texture, tight grid spacing.',
      'Pastel Palette': 'Convert this photo into a perler bead pattern using only soft pastel colors. Light pink, baby blue, mint green, lavender, peach, cream, soft yellow beads. Visible circular bead grid pattern, gentle and dreamy color palette. Each bead is a single pastel shade, creating a soft, delicate pixel art effect.',
      ...
      'Monochrome': 'Convert this photo into a monochrome perler bead pattern using only black, white, and shades of gray beads. Visible circular bead grid, high contrast grayscale pixel art. Each bead is a single shade from pure white to pure black, creating a dramatic black-and-white bead mosaic.',
      'Earth Tones': 'Convert this photo into a perler bead pattern using natural earth tone colors. Terracotta, olive green, brown, tan, rust, cream, forest green, burnt sienna beads. Visible circular bead grid with warm, natural color palette. Each bead is an earthy organic shade.',
      'Rainbow': 'Convert this photo into a perler bead pattern using full rainbow spectrum colors. Vibrant red, orange, yellow, green, blue, indigo, violet beads with all color transitions. Visible circular bead grid, colorful and joyful. Each bead is a bright, saturated rainbow shade.',
      'Vintage': 'Convert this photo into a perler bead pattern using muted vintage retro colors. Dusty rose, sage green, mustard yellow, faded denim blue, burnt orange, cream, mauve beads. Visible circular bead grid with nostalgic, retro-inspired color palette. Slightly desaturated tones.',
    };
    return stylePrompts[selectedPerlerBeadStyle.name] || `Convert this photo into a perler bead pattern in ${selectedPerlerBeadStyle.name} style.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPerlerBeadStyle ? { perlerBeadStyle: selectedPerlerBeadStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate bead pattern. Please try again.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Perler bead pattern generated successfully');
        ...
        if (statusData.status === 'failed') {
          setError(statusData.error || 'Failed to generate bead pattern. Please try again.');
          return;
        }
      }
      ...
      if (error.name === 'TypeError' && error.message?.includes('fetch')) {
        ...
        setError('Failed to generate bead pattern. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                {/* Perler bead style preset selector */}
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {perlerBeadStylePresets.map((preset) => {
                      const isSelected = selectedPerlerBeadStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedPerlerBeadStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedPerlerBeadStyle
                        ? t('input.preset.summary.selected', { name: selectedPerlerBeadStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated perler bead pattern"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-perler-bead-pattern.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🔲', before: `${R2_BASE}/ai-perler-bead-pattern/cases/case-1-before.png`, after: `${R2_BASE}/ai-perler-bead-pattern/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-perler-bead-pattern/cases/case-2-before.png`, after: `${R2_BASE}/ai-perler-bead-pattern/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-perler-bead-pattern/cases/case-3-before.png`, after: `${R2_BASE}/ai-perler-bead-pattern/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Perler Bead Pattern Preview"
          />
        </>
      )}
      <LoginModal
