...
export interface AsciiStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiAsciiArtGeneratorExperienceProps {
  asciiStylePresets: AsciiStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';

const beforeImage = `${R2_BASE}/ai-ascii-art-generator/feature/before.png`;
const afterImage = `${R2_BASE}/ai-ascii-art-generator/feature/after.png`;

export default function AiAsciiArtGeneratorExperience({ asciiStylePresets }: AiAsciiArtGeneratorExperienceProps) {
  const t = useTranslations('aiAsciiArtGenerator');
  ...
  const [selectedAsciiStyle, setSelectedAsciiStyle] = useState<AsciiStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedAsciiStyle) {
      return 'Convert the image into ASCII art using standard characters. Preserve the overall composition and tonal range.';
    }
    if (selectedAsciiStyle.name === 'Classic ASCII') {
      return 'Convert the image into classic ASCII art using standard printable characters (@, #, S, %, ?, *, +, ;, :, ,, ., space). Use dense characters for dark areas and sparse for light. Maintain the original composition.';
    }
    return `Convert the image into ${selectedAsciiStyle.name} style ASCII art. The result should look visually faithful to the original subject while rendered entirely in the ${selectedAsciiStyle.name} character style. Preserve the overall composition and tonal balance.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedAsciiStyle ? { asciiStyle: selectedAsciiStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate ASCII art preview.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('ASCII art transformation applied successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate ASCII art preview. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="ascii-art-generator-upload"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {asciiStylePresets.map((preset) => {
                      const isSelected = selectedAsciiStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedAsciiStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedAsciiStyle
                        ? t('input.preset.summary.selected', { name: selectedAsciiStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated ASCII art preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-ascii-art-generator.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '💻', before: `${R2_BASE}/ai-ascii-art-generator/cases/case-1-before.png`, after: `${R2_BASE}/ai-ascii-art-generator/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-ascii-art-generator/cases/case-2-before.png`, after: `${R2_BASE}/ai-ascii-art-generator/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-ascii-art-generator/cases/case-3-before.png`, after: `${R2_BASE}/ai-ascii-art-generator/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI ASCII Art Generator Preview"
          />
        </>
      )}
      <LoginModal
