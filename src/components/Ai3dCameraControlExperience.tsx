...
export interface CameraAnglePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface Ai3dCameraControlExperienceProps {
  cameraPresets: CameraAnglePresetAsset[];
}

const PROMPT_MAP: Record<string, string> = {
  'Front View': 'Generate a front-facing view of the subject in this image. Position the camera directly in front, looking straight at the subject. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. The subject should appear as if photographed from a standard front-facing camera angle.',
  'Left Side': 'Generate a left side profile view of the subject in this image. Rotate the camera 90 degrees to the left, showing the subject\'s left profile. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. The subject should appear as if photographed from the left side.',
  'Right Side': 'Generate a right side profile view of the subject in this image. Rotate the camera 90 degrees to the right, showing the subject\'s right profile. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. The subject should appear as if photographed from the right side.',
  'Back View': 'Generate a rear view of the subject in this image. Position the camera behind the subject, showing the back of the head, body, and clothing. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. The subject should appear as if photographed from directly behind.',
  'Top Down': 'Generate a top-down bird\'s eye view of the subject in this image. Position the camera directly above, looking straight down at the subject. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. The subject should appear as if photographed from a high overhead angle.',
  'Low Angle': 'Generate a dramatic low-angle view of the subject in this image. Position the camera below the subject, looking upward. This creates a powerful, imposing perspective. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. Cinematic low-angle photography.',
  'Left Three-Quarter': 'Generate a left three-quarter view of the subject in this image. Position the camera at approximately 45 degrees to the left of center, showing both the front and left side of the subject. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. Classic portrait three-quarter angle.',
  'Right Three-Quarter': 'Generate a right three-quarter view of the subject in this image. Position the camera at approximately 45 degrees to the right of center, showing both the front and right side of the subject. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. Classic portrait three-quarter angle.',
  'Close-Up': 'Generate a tight close-up shot of the subject in this image. Move the camera very close to capture fine details — facial features, textures, and small details become prominent. Maintain the exact same subject identity, lighting conditions, and visual style. Intimate close-up photography with shallow depth of field.',
  'Wide Shot': 'Generate a wide-angle shot pulling back from the subject in this image. Position the camera further away to show the subject within a broader context, revealing more of the environment and surroundings. Maintain the exact same subject identity, clothing, lighting conditions, and visual style. Wide establishing shot.',
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/feature/after.png';

export default function Ai3dCameraControlExperience({ cameraPresets }: Ai3dCameraControlExperienceProps) {
  const t = useTranslations('ai3dCameraControl');
  ...
  const [selectedPreset, setSelectedPreset] = useState<CameraAnglePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    const customInstruction = cameraInstruction.trim()
      ? ` Additional camera instructions: ${cameraInstruction.trim()}`
      : '';
    if (!selectedPreset) {
      return `Change the camera angle of this image to show the subject from a different perspective. Maintain the exact same subject identity, style, lighting, and key details while shifting the viewpoint.${customInstruction}`;
    }
    const base = PROMPT_MAP[selectedPreset.name] || `Generate a ${selectedPreset.name} of the subject in this image. Maintain the exact same subject identity, clothing, lighting conditions, and visual style.`;
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedPreset ? { cameraAngle: selectedPreset.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate camera angle.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Camera angle generated successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate camera angle. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label htmlFor="camera-instruction" className="block text-sm font-semibold text-slate-900 mb-2">
                    {t('input.cameraInstruction.label')}
                  </label>
                  <textarea
                    id="camera-instruction"
                    value={cameraInstruction}
                    onChange={(e) => setCameraInstruction(e.target.value)}
                    placeholder={t('input.cameraInstruction.placeholder')}
                    maxLength={300}
                    rows={3}
                  ...
                  />
                  <p className="mt-1 text-xs text-slate-500">{t('input.cameraInstruction.hint')}</p>
                </div>
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-3d-camera-control.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🎥', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/cases/case-1-after.png' },
              { key: 2, icon: '🔄', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-3d-camera-control/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI 3D Camera Control Preview"
          />
        </>
      )}
      <LoginModal
