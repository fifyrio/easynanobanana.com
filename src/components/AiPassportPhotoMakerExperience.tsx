...
export interface PassportSizePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiPassportPhotoMakerExperienceProps {
  passportSizePresets: PassportSizePresetAsset[];
}

type BackgroundColor = 'white' | 'blue' | 'red';

const BACKGROUND_COLORS: Record<BackgroundColor, { hex: string; label: string; promptText: string }> = {
  white: { hex: '#FFFFFF', label: 'White', promptText: 'pure white (#FFFFFF)' },
  blue: { hex: '#2E78D2', label: 'Blue', promptText: 'official passport blue (#2E78D2)' },
  red: { hex: '#D7332B', label: 'Red', promptText: 'official passport red (#D7332B)' },
};

const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/feature/after.png';

export default function AiPassportPhotoMakerExperience({ passportSizePresets }: AiPassportPhotoMakerExperienceProps) {
  const t = useTranslations('aiPassportPhotoMaker');
  ...
  const [selectedPreset, setSelectedPreset] = useState<PassportSizePresetAsset | null>(passportSizePresets[0] ?? null);
  ...
  const buildPrompt = () => {
    ...
    if (!preset) {
      return `Transform this photo into a compliant passport photo with a ${bg.promptText} background. Crop to head and shoulders, neutral expression, even soft lighting.`;
    }
    return [
      `Transform this photo into an official ${preset.name} passport-style portrait photo, sized ${preset.spec} (${preset.widthPx}x${preset.heightPx} pixels).`,
      `Replace the background with a perfectly uniform ${bg.promptText} backdrop with no gradient, shadow, or texture.`,
      `Crop to a centered head-and-shoulders composition. Face must be front-facing, looking directly at the camera, with a neutral relaxed expression, mouth closed, eyes open and clearly visible, no glasses glare.`,
      `Keep the person's identity, skin tone, hair, and facial features fully intact.`,
      `Apply even, soft, shadow-free studio lighting. No accessories obscuring the face. No filters.`,
      `Output must follow strict passport photo compliance for ${preset.name}.`,
    ].join(' ');
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Pick closest supported aspect ratio for output
      const ratio = preset && preset.widthPx === preset.heightPx
        ? '1:1'
        : preset && preset.heightPx > preset.widthPx
          ? '2:3'
          : '3:2';
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          ...
          imageUrls,
          metadata: preset
            ? { passportSize: preset.name, dimensions: `${preset.widthPx}x${preset.heightPx}`, backgroundColor }
            : { backgroundColor },
          aspectRatio: ratio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate passport photo.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Passport photo generated successfully');
  ...
  const previewAspectRatio = selectedPreset
    ? `${selectedPreset.widthPx}/${selectedPreset.heightPx}`
    : '1/1';
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
          ...
          <div className="grid items-start gap-8 lg:grid-cols-2">
            ...
            <div className="rounded-[32px] border border-[#FFE58F] bg-white/90 shadow-[0_40px_120px_rgba(247,201,72,0.25)] p-6 sm:p-10 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-900 mb-2">{t('input.background.label')}</p>
                    ...
                    {(Object.keys(BACKGROUND_COLORS) as BackgroundColor[]).map((color) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          />
                          {t(`input.background.options.${color}`)}
                        </button>
                      );
                    })}
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:hidden">
                    {passportSizePresets.map((preset) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-[9px] text-slate-400">{preset.spec}</div>
                        </button>
                      );
                    })}
                  </div>
                      ...
                      {passportSizePresets.map((preset) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-[10px] text-slate-400">{preset.spec}</div>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedPreset
                        ? t('input.preset.summary.selected', { name: selectedPreset.name, spec: selectedPreset.spec })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated passport photo preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-passport-photo.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '🛂', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/cases/case-1-after.png' },
              { key: 2, icon: '🌐', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-passport-photo-maker/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
                  ...
                  <div className="relative rounded-[28px] overflow-hidden border border-[#FFE7A1] bg-[#FFF3B2]/40 aspect-[4/3] flex items-center justify-center">
                    <div className="grid grid-cols-2 gap-2 p-3 w-full h-full">
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.before} alt="Before passport conversion" className="w-full h-full object-cover" />
                        <span className="absolute bottom-1 left-1 bg-black/50 text-white text-xs px-1.5 py-0.5 rounded">Before</span>
                      </div>
                      <div className="relative rounded-xl overflow-hidden">
                        <img src={card.after} alt="After passport conversion" className="w-full h-full object-cover" />
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Passport Photo Preview"
          />
        </>
      )}
      <LoginModal
