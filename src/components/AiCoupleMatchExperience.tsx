...
export interface CoupleMatchPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiCoupleMatchExperienceProps {
  coupleMatchPresets: CoupleMatchPresetAsset[];
}
...
const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/feature/after.png';

const PROMPT_MAP: Record<string, string> = {
  'Love Score': 'Using the 2 uploaded face photos, create a romantic couple compatibility analysis image. Show both faces side by side with a large heart-shaped love score meter in the center displaying a percentage, compatibility bars for chemistry, humor, loyalty, and passion, pink and red decorative elements, sparkle effects, and a fun romantic verdict banner.',
  'Baby Prediction': 'Using the 2 uploaded face photos as parents, generate a realistic AI-predicted baby face that blends features from both parents. Show the baby portrait in the center with subtle feature-mapping lines connecting inherited traits from each parent. Include a cute baby frame, soft pastel colors, and labels showing which features came from which parent.',
  'Celebrity Couple': 'Using the 2 uploaded face photos, determine which famous celebrity couple they most resemble. Generate a glamorous side-by-side layout showing the uploaded couple next to their celebrity match, with a gold "Celebrity Couple Match" banner, resemblance percentage, and red carpet themed decorations.',
  'Anime Couple': 'Using the 2 uploaded face photos, transform both people into a beautiful anime/manga couple illustration. Convert both faces to matching anime art style with big expressive eyes, stylized hair, and romantic anime atmosphere with cherry blossoms or sparkles. Keep facial resemblance to the originals.',
  'Wedding Portrait': 'Using the 2 uploaded face photos, create a romantic wedding portrait composite. Place both faces into an elegant wedding scene — one in wedding dress, one in suit/tuxedo. Add soft romantic lighting, flower decorations, a wedding arch, and dreamy bokeh background. Professional wedding photography style.',
  'Growing Old Together': 'Using the 2 uploaded face photos, age both faces to show how they would look as an elderly couple together. Generate realistic aged versions of both people side by side — wrinkles, gray hair, warm smiles. Add a heartwarming "Together Forever" banner, timeline showing years together, and warm golden lighting.',
  'Face Merge': 'Using the 2 uploaded face photos, create a single merged face that perfectly blends features from both people. The result should be one face that is a 50/50 blend — eyes from one, nose from the other, mixed skin tone and hair. Show the blend result large in center with small source faces in corners and blend percentage indicators.',
  'Couple Avatar': 'Using the 2 uploaded face photos, create cute matching chibi/cartoon couple avatars. Convert both faces into adorable chibi-style characters with big heads, tiny bodies, matching outfits, and cute poses. Kawaii aesthetic with pastel colors, hearts, and a fun couple frame.',
};

export default function AiCoupleMatchExperience({ coupleMatchPresets }: AiCoupleMatchExperienceProps) {
  const t = useTranslations('aiCoupleMatch');
  ...
  const [selectedPreset, setSelectedPreset] = useState<CoupleMatchPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedPreset) {
      return 'Using the 2 uploaded face photos, create a fun romantic couple compatibility image with a love score, matching analysis, and romantic decorations.';
    }
    return PROMPT_MAP[selectedPreset.name] || `Using the 2 uploaded face photos, create a ${selectedPreset.name} couple match image.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          ...
          metadata: selectedPreset
            ? { coupleMatchStyle: selectedPreset.name, faceCount: imageUrls.length }
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate couple match.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('AI couple match generated successfully');
      ...
      if (err instanceof Error) {
        if (err.name === 'TypeError' && err.message.includes('fetch')) {
          ...
          setError(err.message || 'Failed to generate couple match.');
        }
      } else {
        setError('Failed to generate couple match.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
                ...
                <div className="rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-5 shadow-inner space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {slots.map((slot, index) => {
                      const inputId = `couple-match-upload-${index}`;
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated couple match preview"
                    fill
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '💑', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/cases/case-1-after.png' },
              { key: 2, icon: '👶', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/cases/case-2-after.png' },
              { key: 3, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-couple-match/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
