...
export interface BabyPresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiBabyGeneratorExperienceProps {
  babyPresets: BabyPresetAsset[];
}
...
const beforeImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/feature/before.png';
const afterImage = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/feature/after.png';

export default function AiBabyGeneratorExperience({ babyPresets }: AiBabyGeneratorExperienceProps) {
  const t = useTranslations('aiBabyGenerator');
  ...
  const [selectedBaby, setSelectedBaby] = useState<BabyPresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    const baseClause = 'Using the 2 uploaded parent photos, generate a realistic baby face that blends the facial features of both parents.';
    if (!selectedBaby) {
      return `${baseClause} Create a cute, healthy-looking baby with natural features inherited from both parents.`;
    }
    return `${baseClause} The baby should have a ${selectedBaby.name} appearance — cute and natural-looking with features inherited from both parents.`;
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const babyStyleHint = selectedBaby
        ? `The requested baby style is "${selectedBaby.name}". Create a realistic, adorable baby face.`
        : '';
      const finalPrompt = `${promptText} ${babyStyleHint} Deliver a photo-realistic AI baby face image powered by Easy Nano Banana.`;
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedBaby
            ? { babyStyle: selectedBaby.name, parentCount: imageUrls.length }
            : { parentCount: imageUrls.length },
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate AI baby image.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('AI baby image generated successfully');
      ...
      if (err instanceof Error) {
        if (err.name === 'TypeError' && err.message.includes('fetch')) {
          ...
          setError(err.message || 'Failed to generate AI baby image. Please try again.');
        }
      } else {
        setError('Failed to generate AI baby image. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                <div className="rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-5 shadow-inner space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {slots.map((slot, index) => {
                      const inputId = `baby-upload-${index}`;
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {babyPresets.map((preset) => {
                      const isSelected = selectedBaby?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedBaby(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedBaby
                        ? t('input.preset.summary.selected', { name: selectedBaby.name })
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '👶', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/cases/case-1-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/cases/case-1-after.png' },
              { key: 2, icon: '✨', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/cases/case-2-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/cases/case-2-after.png' },
              { key: 3, icon: '⚡', before: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/cases/case-3-before.png', after: 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases/ai-baby-generator/cases/case-3-after.png' },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          <ShareModal
            ...
            imageUrl={generatedImage}
            description={description || 'AI Baby Generator by Easy Nano Banana'}
          />
          <ImagePreviewModal
            isOpen={showPreviewModal}
            onClose={() => setShowPreviewModal(false)}
            imageUrl={generatedImage}
            title="AI Baby Preview"
          />
        </>
      )}
      <LoginModal
