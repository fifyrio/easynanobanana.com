...
export interface MuscleStylePresetAsset {
  displaySrc: string;
  referenceSrc: string;
  fileName: string;
  name: string;
...
interface AiMuscleGeneratorExperienceProps {
  muscleStylePresets: MuscleStylePresetAsset[];
}

const R2_BASE = 'https://pub-103b451e48574bbfb1a3ca707ebe5cff.r2.dev/showcases';
const beforeImage = `${R2_BASE}/ai-muscle-generator/feature/before.png`;
const afterImage = `${R2_BASE}/ai-muscle-generator/feature/after.png`;

export default function AiMuscleGeneratorExperience({ muscleStylePresets }: AiMuscleGeneratorExperienceProps) {
  const t = useTranslations('aiMuscleGenerator');
  ...
  const [selectedMuscleStyle, setSelectedMuscleStyle] = useState<MuscleStylePresetAsset | null>(null);
  ...
  const buildPrompt = () => {
    if (!selectedMuscleStyle) {
      return 'Keep the subject\'s physique exactly as it is. No body modification.';
    }
    if (selectedMuscleStyle.name === 'Slim') {
      return 'Transform the subject to have a slim, lean physique with minimal body fat and subtle muscle tone. The result should look natural and realistic. Keep the same identity and features.';
    }
    if (selectedMuscleStyle.name === 'Average') {
      return 'Transform the subject to have an average, healthy physique that looks natural and realistic. Keep their identity, clothing, pose, and background identical.';
    }
    return `Transform the subject's physique to match a realistic ${selectedMuscleStyle.name} body type. The transformation should look natural and genuine, not exaggerated or artificial. Keep their identity, clothing, pose, and background identical.`;
  };
    ...
    try {
      ...
      const response = await fetch('/api/generate-image', {
        ...
        body: JSON.stringify({
          prompt: finalPrompt,
          imageUrls,
          metadata: selectedMuscleStyle ? { muscleStyle: selectedMuscleStyle.name } : undefined,
          aspectRatio,
        }),
      });
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          setError(data.error || 'Failed to generate muscle transformation.');
        }
        return;
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        if (statusData.status === 'completed') {
          if (statusData.resultUrls && statusData.resultUrls.length > 0) {
            ...
            setDescription('Muscle transformation applied successfully');
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        ...
        setError('Failed to generate muscle transformation. Please try again.');
      }
    } finally {
      ...
      <main className="min-h-screen bg-gradient-to-b from-white via-[#FFFBEA] to-white text-slate-900 pb-16">
              ...
              <div className="space-y-6">
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <label
                      htmlFor="muscle-generator-upload"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2 sm:hidden">
                    {muscleStylePresets.map((preset) => {
                      const isSelected = selectedMuscleStyle?.referenceSrc === preset.referenceSrc;
                      return (
                        <button
                          type="button"
                          key={preset.referenceSrc}
                          onClick={() => setSelectedMuscleStyle(isSelected ? null : preset)}
                  ...
                  <div className="mt-3 rounded-2xl border border-[#FFE7A1] bg-[#FFFBF0] p-4 text-xs text-slate-700">
                    ...
                    <p>
                      {selectedMuscleStyle
                        ? t('input.preset.summary.selected', { name: selectedMuscleStyle.name })
              ...
              <div className="rounded-[36px] border border-[#FFE7A1] bg-white shadow-[0_40px_140px_rgba(196,147,18,0.25)] p-4">
                ...
                >
                  <Image
                    ...
                    alt="AI generated muscle transformation preview"
                    fill
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row">
                      <FreeOriginalDownloadButton
                        ...
                        filename="ai-muscle-generator.png"
        ...
        <section className="bg-gradient-to-b from-[#FFF7DA] via-white to-[#FFF7DA] text-slate-900">
          ...
          <div className="max-w-6xl mx-auto px-4 pb-16 space-y-6">
            {[
              { key: 1, icon: '💪', before: `${R2_BASE}/ai-muscle-generator/cases/case-1-before.png`, after: `${R2_BASE}/ai-muscle-generator/cases/case-1-after.png` },
              { key: 2, icon: '✨', before: `${R2_BASE}/ai-muscle-generator/cases/case-2-before.png`, after: `${R2_BASE}/ai-muscle-generator/cases/case-2-after.png` },
              { key: 3, icon: '⚡', before: `${R2_BASE}/ai-muscle-generator/cases/case-3-before.png`, after: `${R2_BASE}/ai-muscle-generator/cases/case-3-after.png` },
            ].map((card, index) => (
              <div
                key={card.key}
      ...
      {generatedImage && (
        <>
          ...
          <ImagePreviewModal
            ...
            imageUrl={generatedImage}
            title="AI Muscle Generator Preview"
          />
        </>
      )}
      <LoginModal
