...
const FEATURE_PATH = '/ai-image-effects/ai-pet-portrait';
...
export default async function AiPetPortraitPage({
  ...
  const tSeo = await getTranslations({ locale, namespace: 'aiPetPortrait.seo' });
  const tFaq = await getTranslations({ locale, namespace: 'aiPetPortrait.faq' });
  ...
  return (
    <>
      ...
      <AiPetPortraitExperience petPortraitStylePresets={presets.petPortraitStyles} />
    </>
  );
}
