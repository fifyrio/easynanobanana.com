...
> 版本 v0.2 · 2026-07-13 · **架构修订：MCP-first，重计算下放用户本地，不购置 worker 服务器**
...
本地 ffmpeg silencedetect → 场景时间戳       （无需 whisper：分镜只需停顿点，
逐场景生成图片 prompt                          ffmpeg 一条命令本地完成）
...
**结论**：用户要的不是又一个生成器，而是一条不用离开网站的流水线。我们已有 80% 的基础设施（生图 API、KIE Seedance 管线、credit 原子扣费、R2 存储），缺的只是编排层。
...
- 全流程异步任务 + 轮询/回调，复用现有 `videos` 表的 pending/processing/completed 状态机模式。
- 任何一步失败只重试该步，不作废整个项目。
...
- TTS：接 KIE 音频模型或 MiniMax/Fish Audio，多音色，输出 mp3 存 R2，按分钟扣 credits。
...
- 薄 MCP server（Streamable HTTP，托管在现有 Vercel 或 Cloudflare Workers），工具即现有 API 的包装：
  `generate_image` / `batch_generate_images` / `tts` / `list_voices` / `get_credits` / `generate_thumbnail`。
- 官方工作流 Skill（Markdown 文件，官网可下载 / npx 一键安装）：教 Claude Code 完整流程——写脚本 → 调 tts → 本地 `ffmpeg silencedetect` 出场景时间戳 → 逐场景写 prompt → 批量调 generate_image → 本地 ffmpeg 合成 1080p mp4（Ken Burns + 字幕）→ 输出成品文件夹（mp4 + 缩略图×3 + 标题/描述/标签）。
...
复用：`user_profiles.credits`、`credit_transactions`、`images`、`videos`、现有 RPC 模式（`deduct_credits_for_image` / `deduct_credits_for_video`）。
...
CREATE TABLE public.api_keys (
  ...
  key_hash      text NOT NULL UNIQUE,    -- 只存哈希，明文仅创建时显示一次
  name          text,
...
CREATE TABLE public.batch_jobs (
  id            uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id       uuid NOT NULL REFERENCES auth.users(id),
  project_id    uuid REFERENCES workflow_projects(id),
  job_type      text NOT NULL,           -- bulk_image | tts | transcribe | compose
...
新增 RPC（镜像现有模式，原子扣费）：`deduct_credits_for_batch(p_user_id, p_amount, p_job_type, p_params)` → 校验余额、插入 batch_jobs（pending）、写 credit_transactions，返回 job_id。失败部分按张数退款（正向 credit_transactions）。
...
| `tts` | text, voiceId | 返回 mp3 URL + 时长 |
| `list_voices` | - | 音色列表 |
...
2. 调 `tts` 生成配音，下载到项目文件夹。
3. 本地 `ffmpeg -af silencedetect=noise=-35dB:d=0.45` 检测停顿 → 与脚本句子对齐 → `scenes.json`（时间戳+文本）。
...
| 参考消耗（10 分钟视频 ≈100 场景） | ≈ 550 credits | 100 张图 400（折扣价）+ TTS 100 + 缩略图 15 + 余量 |

失败退款：批量任务按失败张数自动退，复用现有 refund txn 模式。MCP 调用与网页共用同一 credits 池。

## 7. 营销联动（Demo 视频脚本要点）

主视频结构照抄 Higgsfield MCP 教程（26w 播放已验证）：

1. 钩子：找到一个月入 $X 的 faceless 频道 → "我用 AI 在 20 分钟内复刻它"。
2. 接入：官网复制 MCP 安装命令 → Claude Code 连接 → 填 API Key（全程 <2 分钟，镜头实拍）。
...
| 周 | 交付 |
|---|---|
| W1-W2 | Phase 1：batch_jobs 表 + /api/bulk + /bulk-generator 页上线 |
| W3 | Phase 2：TTS API + api_keys 表 + Key 管理页 |
| W4-W5 | Phase 3：MCP server（/api/mcp）+ 工作流 Skill + /mcp 文档落地页 |
| W6 | 录制 demo 视频（对标 Higgsfield 教程结构）、SEO 落地页、发布 |

## 10. 成功指标

- 北极星：完成整条流水线（到导出）的项目数/周。
- 辅助：批量生图张数、流水线完成率（①→⑥漏斗）、credit 消耗中工作流占比、demo 视频带来的注册转化。
